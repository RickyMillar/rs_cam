...
**Drill-specific thresholds (Step 3 PR2)** — read from `cut_trace.drill_summaries` keyed by `toolpath_id`, gate verdicts from `get_tool_load_report().per_toolpath[].drill_gates`:
...
- `ToolpathStats` (`crates/rs_cam_core/src/compute/config.rs`) also carries twelve report-only generation findings, surfaced through `narrate_toolpath` and the diagnostics list: `truncated_core_mm2` (renamed from `standing_material_mm2` in wave 16, 2026-08-04 — the old spelling survives only as a legacy JSON key), `untouched_material_mm2`, `reached_uncut_estimate_mm2`, `dropped_band`, `clipped_band`, `tip_float`, `deprecated_dial`, `derived_stepovers`, `ramp_reach_clamp`, `claims_reference`, `retract_trips`, `zero_removal` (wave 16: a rest pass whose emitted cutting geometry never gets under the stock the prior op left — it costs full price in motion and removes nothing; a report, not a refusal). Ten share one contract: `None` means **not measured** (the generator path never ran that check), `Some(0.0)` means measured and clean — never coerce an absent value to zero. Two are deliberately outside it and say so in their own docs: `derived_stepovers` is a `Vec` (empty = nothing derived a stepover, not "measured zero"), and `zero_removal`'s `None` conflates "not measured" with "nothing to report", because it supports no ratio. No gate consumes any of these; they are report-only.
