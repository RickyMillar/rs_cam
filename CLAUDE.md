...
- **Analytic drill removal needs the frame's cut direction, and a drill on a side-face setup does nothing.** `TriDexelStock::apply_drill_op(&drill_op, direction)` removes material *above* the tip envelope only when the tool advances downward in the grid it is applied to. A `DrillHole` carries no axis — it is an XY centre plus `top_z`/`bottom_z` — so after `group_drill_op_to_global` maps a `FaceUp::Bottom` setup (`z → H − z`) the pair arrives **inverted**, and `top_z <= bottom_z` is then correct, not corrupt. Never `min`/`max` the pair and never infer the direction from their ordering (a zero-depth hole is degenerate); the answer is the group's `StockCutDirection`, which already rides both call paths. Pre-fix (G-DRILLFLIP, found by an operator watching the viewport) a flipped blind hole carved the exact **complement** of its own band — plausible-looking and wrong, and the global stock is what `StockSource::FromRemainingStock` reads. Lateral setups (`FaceUp::{Front,Back,Left,Right}`) **abstain**: `DrillHole` cannot express a hole whose axis is global X or Y, so `DrillRemovalReport::unrepresentable_axis` is set and nothing is carved (G-DRILLLATERAL, open). Note lateral support is **partial across the board** — milling works (the X/Y dexel grids are allocated lazily), 2D polygon ops collapse to a degenerate line (G-SIDEFACE-POLYCOLLAPSE, open), drilling abstains.
