...
- [x] 5.1b GUI Phase 4+5: All 2.5D toolpaths — OperationConfig enum with 7 operation types (Pocket, Profile, Adaptive, VCarve, Rest Machining, Inlay, Zigzag), each with full parameter panels and DragValue editing. Background compute worker handles all types via depth_stepped_toolpath. Profile supports tabs (count/width/height). Adaptive has tolerance/slot clearing/min cut radius. VCarve/Inlay validate V-Bit tool. Rest machining resolves previous tool radius. SVG import for 2D geometry. Toolpath line rendering with Z-depth coloring. "+ Add Toolpath" menu with all operation types.
