...
- [x] 5.1e GUI Phase 9+10: Polish — keyboard shortcuts (Ctrl+Z/Ctrl+Shift+Z undo/redo, Ctrl+S save, Ctrl+Shift+E export), undo/redo for stock config changes, DXF import (load_dxf), collision detection on background thread (check_collisions_interpolated, ToolAssembly from tool config), collision count in status bar, Generate All, enhanced status bar (computing indicator, toolpath count, SIM badge, collision warnings, dirty flag), File/Edit/Toolpath/Simulation/View menus
