...
- [x] 5.1b GUI Phase 4+5: All 2.5D toolpaths — 7 operation types (Pocket, Profile, Adaptive, VCarve, Rest, Inlay, Zigzag), full parameter panels, background compute worker, depth stepping, SVG import, toolpath rendering with Z-depth coloring
- [x] 5.1c GUI Phase 6+8: All 3D operations + G-code export — 7 more operation types (3D Finish, 3D Rough, Waterline, Pencil, Scallop, Steep/Shallow, Ramp Finish), tool builder for dyn MillingCutter (all 5 tool types), mesh+spatial index for 3D ops, G-code export via File menu (emit_gcode_phased with selected post-processor), all 14 operation types fully wired end-to-end
- [x] 5.1d GUI Phase 7: Material removal simulation — heightmap-based simulation on background worker thread (Heightmap::from_bounds, simulate_toolpath for each enabled toolpath), heightmap_to_mesh with wood-tone vertex colors, SimMeshGpuData with smooth vertex normals, simulation mesh replaces raw STL in viewport when active, Simulate/Reset buttons in viewport overlay + Simulation menu, resolution 0.25mm default
- [x] 5.1e GUI Phase 9+10: Polish — keyboard shortcuts, DXF import, collision detection (red markers in viewport), parameter validation, tooltips on all params, debounced auto-regen for 2.5D ops, wireframe/shaded toggle, dressup config (entry/dogbone/lead/link/arc/feed), toolpath duplicate/enable/reorder, TOML job save/load (compatible with CLI format), expanded undo/redo (stock/post/tool/toolpath params), simulation play/pause controls, enhanced status bar, full menu system
