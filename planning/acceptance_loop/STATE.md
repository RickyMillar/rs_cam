...
| [F-034](findings/F-034-machine-kinematics-cycle-time.md) | Acceleration-aware cycle time estimator — replaces `distance/feed` with kinematics-integrator | sim | medium | S-M | **verified 2026-05-26** (commits `ae58f55` + arc-fitter `2db19c2` + calibration); model predicts wanaka Back Rough within ±15 % of measured 827 s |
...
| [F-036c](findings/F-036c-feed-modulation-real-machine-calibration.md) | Real-machine cycle-time calibration on Shapeoko XXL (F-036 Piece F) | verify | low | S code-side + user wall-clock | **verified 2026-05-26** — wanaka Back Rough measured 13:47 unmod / 20:24 mod, 0 GRBL errors, modulator in protective mode (band-ceiling clamp) |
| [F-038](findings/F-038-adaptive3d-entry-fragmentation.md) | Adaptive3d AgentSearch entry-plunge fragmentation filter — drops sub-15 mm cut groups + coalesces back-to-back entries | sim | medium | M | **landed 2026-05-27** — Wanaka Back Rough entry events 131 → 59 (−55 %), cycle time model 789 → 576 s (−27 %). F-034/F-036c calibration anchors need re-bench. |
| [F-038b](findings/F-038b-adaptive3d-keep-tool-down.md) | Adaptive3d keep-tool-down link between cut groups — dexel-heightfield-safe stay-down replacing retract-rapid-plunge | sim | medium | L | **landed 2026-05-27** — wanaka rapid_mm 28608 → 25907 (−9.4 %), predicted runtime 3218 → 3044 s (−5.4 %). Included latent `tsp::rebuild_group` retract-invariant fix exposed by F-038b. |
| [F-039](findings/F-039-constrained-max-feed-modulation.md) | Constrained-max feed modulation + transparency layers (Layer 1 + 3 + algorithm + CLI flags) — supersedes F-036's "target band-mid" default | sim + diagnostics | medium | L | **landed 2026-05-27** — wanaka cycle ConstrainedMax@1.0 = BandMid (binding constraint isn't chipload-max here). Aggressiveness 1.3 trims 16 min. Layer 2 G-code comments deferred to F-039c. Layer 1 viz panel landed alongside user's in-flight feeds/viz work. |
| [F-039a](findings/F-039a-modulation-feed-heatmap.md) | 3D feed-rate heatmap on toolpath visualisation — depends on F-039's per-move modulated-feeds map | viz | low | M | open — design only |
| [F-040](findings/F-040-lead-in-out-feed-breakout.md) | Lead-in / lead-out feed-rate breakout (separate `DressupConfig` knobs + `MoveIntent::{LeadIn,LeadOut}` + F-039 modulator skip) | dressup + emitter | low | S-M | **landed 2026-05-27** — default `None` byte-identical; opt-in via `lead_in_feed_rate = Some(N)` / `lead_out_feed_rate = Some(N)`. Bench-verified F500 feed change audible. |
| [F-040a](findings/F-040a-lead-in-out-geometry.md) | Lead-in geometry: pre-position rapid + pure-Z plunge + tangent arc (classic shape) — replaces pre-F-040a diagonal feed | dressup geometry | medium | S | **landed 2026-05-27** — bench observed variant 6 "looks like plunge/retract"; F-040 feeds worked but geometry was already broken pre-F-040. |
...
- 2026-05-27 — **F-038b / F-039 / F-040 bench verified on Shapeoko XXL.** User ran 6-variant bench batch (`planning/feed_modulation_calibration/bench_batch_2026-05-27/`) in the air (no real-wood cuts) with `--inject-shapeoko-kinematics`. Wall-clock results (project --summary prediction in parens; F-034 ±15% target comfortably beaten — sim consistently over-predicts by 3-4%): **v1 unmodulated 7:22 (7:39, +3.8%), v2 F-036 BandMid 13:45 (14:16, +3.8%), v3 F-039 ConstrainedMax @ 1.0 13:37 (14:08, +3.8%), v4 CMax @ 1.3 11:16 (11:43, +4.0%), v5 CMax @ 1.0 stay-down OFF 15:57 (16:03, +0.6%), v6 small profile 0:35 (0:33, −5.7%)**. nc-time replay tool (`cargo run -p rs_cam_cli -- nc-time <file.nc>`) cross-check predictions are systematically 5-12% lower (no setup-overhead modelling) but **relative deltas between variants agree within 1-2% between nc-time and project --summary**. Workstream claims all confirmed on hardware: **F-038b stay-down savings (v3→v5): predicted +14.3%, actual +14.7% ✓**; **F-039 aggressiveness 1.3 (v3→v4): predicted −17.1%, actual −17.3% ✓**; **ConstrainedMax ≈ BandMid on wanaka (v2↔v3): predicted −0.9%, actual −1.0% ✓** (binding constraint on wanaka is deflection / kinematic-reach, not chipload-max — F-039b sub-finding remains open to characterise the binding distribution); **F-040 audible feed change at lead-in/out: confirmed ✓**. Bench surfaced one new finding: **F-040a** — variant 6 ran as plunge/retract rather than classic tangent lead-in. F-040's feeds worked (audible F500); the geometry was already broken pre-F-040. Sub-finding landed in the same session (see next entry). Zero GRBL errors across all 6 variants — arc-fitter fix (`2db19c2`) holds on real hardware. F-034 calibration anchor for Back Rough Z = 459 s (the prior 827 s anchor from 2026-05-26 is stale since F-038/F-038b shortened the toolpath); the `NEEDS-RE-MEASURE` markers on `tests/machine_kinematics_cycle_time_f034.rs` and `tests/feed_modulation_cycle_time_f036c.rs` can now be re-tightened to the new measurements. Bench was air-cut only — surface-finish comparisons (F-039 deflection-binding theory, F-040 lead-in mark cleanliness) deferred to the next real-wood session.
- 2026-05-27 — **F-040a landed: lead-in geometry rewrite to classic shape.** Bench observation from variant 6 (the user noted "plunge/retract, NOT classic lead in/out") led directly to this fix. `apply_lead_in_out_with_feeds` in `crates/rs_cam_core/src/dressup.rs` now emits the three-move classic lead-in: (1) pre-position Rapid at safe_z above `lead_start` (tagged `MoveIntent::LeadIn`), (2) pure-Z plunge feed to cut Z at the plunge rate (tagged `MoveIntent::EntryPlunge`), (3) tangent arc steps at `lead_in_feed_rate.unwrap_or(plunge_rate)` (tagged `LeadIn`). Pre-F-040a the planner emitted a single diagonal feed from `(cut_start.xy, safe_z)` straight to `(lead_start.xy, cut_z)` followed by the arc — geometrically a slanted plunge, not a tangent entry. The pre-position rapid reads `safe_z` from `moves[i - 1].target.z` (the preceding rapid's Z). Net effect on move count: +1 per lead-in. F-040's 4 existing tests updated to filter `LeadIn ∩ Linear` (the LeadIn set now contains both the new Rapid and the arc Linear moves) — feeds still assert correctly. New 5th test `lead_in_geometry_is_pre_position_then_pure_z_plunge_then_arc` asserts the move pattern directly from the IR (verifiable without bench). F-037 smoke diff: **no regressions** (none of 18 smoke cases use `lead_in_out = true`). Workspace clippy clean. Variant 6 regenerated in the bench bundle + USB — G-code now shows the classic shape: `G0 → G0 (pre-position) → G1 Z-down at plunge feed → arc at lead-in feed → cut`. CAMotics 1.2.0 GRBL-parse clean (also patched `validate.sh` to set `--entrypoint /usr/bin/gcodetool` explicitly — the prior `docker commit` to add libqt5websockets5 had clobbered the image's ENTRYPOINT). Acceptance test: `cargo test --test lead_in_out_feed_rates_f040 lead_in_geometry`. F-040 finding doc would have called this the right shape; the bug pre-existed F-040 and only surfaced when F-040 made the lead-in feed audibly distinct from the cutting feed.
- 2026-05-27 — **F-040 landed: lead-in / lead-out feed-rate breakout.** New `DressupConfig.lead_in_feed_rate: Option<f64>` + `lead_out_feed_rate: Option<f64>` (default `None`; opt-in per-job). New `MoveIntent::{LeadIn, LeadOut}` enum variants. `apply_lead_in_out_with_feeds(annotated, radius, lead_in_feed_rate, lead_out_feed_rate)` is the new public API; legacy `apply_lead_in_out` delegates with `None`/`None`. F-039 modulator's `should_skip_modulation` now also returns `true` for `LeadIn` / `LeadOut` (treat operator's explicit lead feeds as user-tuned). Behaviour clarification vs design doc: default fallback is the *plunge* feed of the move being replaced (preserving pre-F-040 behaviour byte-identical), not the cut feed — operators who want cut-feed lead-in set `lead_in_feed_rate = Some(<cut_feed>)` explicitly. Side effect of the 16-byte `DressupConfig` growth: `crates/rs_cam_viz/src/state/history.rs::UndoAction::ToolpathParamChange` tipped over the `clippy::large_enum_variant` diff threshold (largest variant 648 bytes) — added `#[allow]` with SAFETY note since UndoAction lives in a bounded history Vec, not a hot loop. Boxing the variant is a possible future refactor. Tests: `crates/rs_cam_core/tests/lead_in_out_feed_rates_f040.rs` 4 tests (apply when set, byte-identical fallback when None, lead-out works, F-039 modulator skips LeadIn/LeadOut). F-037 smoke diff: **no regressions** (18 cases unchanged — none of the smoke matrix has `lead_in_out = true` per `for_role`). Workspace clippy clean. F-024 / F-028 / F-036b / F-038b / F-039 regression nets green. Bench fixture variant 6 generated alongside (small profile op with `lead_in_feed_rate = 500.0`, `lead_out_feed_rate = 4500.0`) — audible feed transition at entry / exit on real hardware. Acceptance test: `cargo test --test lead_in_out_feed_rates_f040`.
- 2026-05-27 — **F-039 landed: constrained-max feed modulation + transparency layers.** Replaces F-036's "target band-mid" heuristic with a per-move constrained-optimization solver in `crates/rs_cam_core/src/feed_modulation.rs` returning `(emitted_feed, BindingConstraint)`. Six constraints checked per move: chipload-max (band ceiling × RPM × flutes × chip-thinning), deflection-max (reuses `tool_load::deflection` cantilever model, 0.2 mm tip cap), power-max (reuses `tool_load::power`, 0.85 × machine power cap), machine-max-feed (`$110/$111/$112` hard cap), kinematic-reach (reuses F-035's `predicted_feeds_for_toolpath`), chipload-min floor (rubbing-prevention). `min()` across all six = emitted feed × aggressiveness, floored at band-min. New `ModulationStrategy::{BandMid, ConstrainedMax}` feature flag on `SimulationOptions` (default flipped to `ConstrainedMax`; `BandMid` kept as fallback). New `SimulationOptions::modulation_aggressiveness: f64` (default 1.0). Three transparency layers per the design doc: **Layer 1** new `ModulationSummary` field on `ToolpathLoadVerdict` (per-toolpath summary with `binding_constraint_distribution`, `moves_touched`, `median_feed_delta_pct`, `aggressiveness`) + viz panel readout in `rs_cam_viz/src/ui/`; **Layer 2** per-move G-code comments **deferred to F-039c** (only doc-comment stub); **Layer 3** `SimulationCutTrace::modulated_feeds: BTreeMap<(toolpath_id, move_idx), (f64, BindingConstraint)>` for downstream viz consumers (F-039a heatmap). New CLI flags on `project` subcommand: `--modulation-strategy {constrained-max,band-mid}` + `--modulation-aggressiveness <f64>`. F-036b's 9 acceptance tests kept passing alongside F-039's new 6 (`crates/rs_cam_core/tests/constrained_max_modulation_f039.rs`). Validation: clippy clean (4 `for_kv_map` lints in the new test file fixed post-agent-handover after agent terminal-crashed during workspace test run — known infinite-loop hazard in our cargo test environment). F-037 smoke diff: **no regressions** (18 cases byte-identical at default ConstrainedMax @ 1.0). All regression nets pass: F-024 / F-026 / F-028 / F-035 / F-036a / F-036b / F-036b1 / F-036c / F-037 / F-038 / F-038b + 12 feed_modulation units + 303 tool_load units + 10 TSP units + 6 F-039 acceptance. **Wanaka full-project sim comparison (resolution 1.0 mm, `--inject-shapeoko-kinematics`)**: unmodulated 5223 s baseline / F-036 BandMid 9925 s / F-039 ConstrainedMax @ 1.0 9915 s (≈ identical to BandMid — binding constraint isn't chipload-max on wanaka, likely deflection or kinematic-reach) / F-039 ConstrainedMax @ 1.3 8935 s (−990 s, −10 % vs aggressiveness 1.0). The design doc's "should be CLOSER to unmodulated 13:47" prediction does not hold on wanaka — F-039b sub-finding opened to investigate the actual binding-constraint distribution. F-039's value here is the transparency surface, not the cycle-time delta. Bench validation batched with F-038b. Agent crossed the "don't touch in-flight viz/feeds" rule (~655 LOC into feeds/* + rs_cam_viz/src/{app,controller,state,ui}/*); user approved committing as-is per scope-decision dialog. Two sub-findings opened: **F-039b** (investigate wanaka binding distribution), **F-039c** (Layer 2 G-code comments). Acceptance test: `cargo test --test constrained_max_modulation_f039`.
- 2026-05-27 — **F-038b landed: adaptive3d keep-tool-down link between cut groups.** New `try_emit_stay_down_link` in `adaptive3d/path.rs` (~145 LOC) probes the mesh heightfield at 10 stations along the candidate XY link line via `point_drop_cutter` and emits three `MoveIntent::Linking` feed moves at `link_z = max(samples, from.z, to.z) + clearance` when (a) `xy_dist <= max_stay_down_distance_mm` (default 8 × tool_diameter — Fusion HSM convention), (b) `link_z <= safe_z`, (c) `link_z <= to.z + cutter.length()`. Rejection at any step falls through to the legacy retract-rapid-plunge. Gated to `EntryStyle3d::Plunge` (Helix/Ramp have their own entry geometry). New `Adaptive3dParams::max_stay_down_distance_mm: Option<f64>` (None ⇒ 8×diam, Some(0.0) disables) + `stay_down_clearance_mm: f64` (default 0.5 mm). Plumbed through `Adaptive3dConfig` → `compute/execute.rs` → `compute/catalog.rs` (op-schema) → CLI `OperationDef`. **Latent bug fixed in same commit (`tsp::rebuild_group`):** when stay-down eliminates a depth-pass's internal rapids, that pass becomes a single segment → `optimize_one_group` fast path → no trailing retract → cutter at low Z → next group's leading rapid emitted as a single diagonal `(low Z) → (next_seg.start.xy, safe_z)` slicing through stock. Root-caused via the `idx == 0` branch in `rebuild_group` only emitting ONE rapid; idx>0 emits TWO (vertical retract + horizontal traverse). Fix: idx==0 branch now conditionally emits the vertical retract first when `result.moves.last().target.z < safe_z`, mirroring idx>0. New test file `crates/rs_cam_core/tests/adaptive3d_keep_down_link_f038b.rs` (522 LOC, 4 tests): stay-down emitted on flat terrain, peak in gap forces retract, default knob caps at 8 × diameter, cutter-length safety guard. **Wanaka full project comparison (resolution 1.0 mm)**: stay-down OFF → ON drops rapid_mm 28608 → 25907 (**−9.4 %**), Back Rough rapid_mm 6468 → 3338 (**−48 %**), 3D Rough 6 rapid_mm 2336 → 943 (**−60 %**), predicted runtime 3218 → 3044 s (**−174 s, −5.4 %, ≈3 min saved**). Smoke baseline diff: **no regressions** (caught the TSP retract regression on AS013 during validation and fixed before commit). All regression nets pass: F-024/F-026/F-028/F-035/F-036b/F-036b1/F-036c/F-038/F-038b + 10 TSP unit tests + 9 F-036b acceptance tests. Workspace clippy + tests clean. F-038b agent landed implementation; main session 2026-05-27 added the TSP fix, ran wanaka comparison, wrote finding doc + STATE entry, committed. Bench verification batched with F-039 to amortize the trip cost. Acceptance test: `cargo test --test adaptive3d_keep_down_link_f038b`.
- 2026-05-27 — **F-038 landed: adaptive3d AgentSearch entry-plunge fragmentation filter.** New `Adaptive3dParams::min_region_cut_length_mm` (default 15.0 mm) gates three filters inside `clear_z_level_agent_2d_slice`: region-level forecast (skip marching-squares regions whose perimeter+adaptive cut length is below threshold), group-level (drop interior `[Rapid, Cut*]` groups in the 2D adaptive output that fail the bar), and post-emission coalescing (collapse back-to-back `Rapid`/`RapidWithFloor` segments produced by engagement subdivision). Wanaka Back Rough .nc impact: entry events 131 → 59 (**−55 %**), zero-cut entries 12 → 0, ≤ 10 mm cut entries 72 → 5, cycle time (model) 789 s → 576 s (−27 %). CAMotics validator green. Smoke baseline diff: **no regressions**. F-024/F-026/F-027/F-028/F-031/F-036b acceptance tests pass byte-identical. F-036c and F-034 calibration tests widened temporarily (NEEDS-RE-MEASURE marker) because their wall-clock references are pre-F-038. Workspace clippy + tests clean. Acceptance test: `cargo test --test adaptive3d_entry_coalescing_f038` (775 → 280 entries on synthetic fixture).
- 2026-05-26 — **F-036c + F-034 verified on real Shapeoko XXL.** User ran the three calibration .nc files dry on the workshop machine (after the F-036c CLI helpers + Docker CAMotics pre-flight validator landed and the arc-fitter fix at `2db19c2` cleared 13 GRBL-rejected arcs in the Back Rough file). Bench results: Pin Drill 1:05 × 3 (identical), Back Rough modulation-off 13:47, modulation-on 20:24, **zero GRBL errors**. Captured user's `$$` ($120/$121 = 500 mm/s², $122 = 270 mm/s², $110/$111 = 10000 mm/min, $112 = 1000 mm/min — non-stock XXL tuning) and added `MachineKinematics::shapeoko_xxl_ricky_tuned()` with blended 350 mm/s² scalar (empirically lands within ±15 % of the Back Rough wall-clock; pure X/Y 500 over-predicts, pure Z 270 under-predicts). Rewrote F-034's `cycle_time_calibrated_against_shapeoko_reference` test to use wanaka Back Rough at 827 s (drill ops bypass `total_runtime_s`, so Pin Drill couldn't anchor the test); unflagged. New `tests/feed_modulation_cycle_time_f036c.rs` adds two F-036c regression tests: modulated/unmodulated ratio sanity envelope [0.5, 1.7] (model 1.608 vs measured 1.480), and modulated cycle prediction within ±25 % of wall-clock (1134 s vs 1224 s = 92.6 %). Both tests skip-by-design when `wanaka_full_tuned.toml` is absent (CI / other devs). **Second bug fixed in this commit**: F-036b's modulation pass ran AFTER `apply_kinematics_cycle_time`, so the trace's `total_runtime_s` reflected pre-modulation commanded feeds. Patched `apply_adaptive_feed_modulation` to re-walk every toolpath through `compute_cycle_time` post-modulation and rewrite the trace's per-toolpath + project-total runtime via `Arc::make_mut`. Outcome interpretation: wanaka's tuned baseline (~3371 mm/min mean) runs **above** the LUT chipload band for hardwood roughing — modulation drops feeds toward band-mid (~1539 mm/min mean) for tool-life safety, not cycle-time win. F-036c's original "≥ 20 % cycle reduction" target was informational; the load-bearing F-036c gate is "modulation runs error-free on real hardware and stays inside the sanity envelope" — that passed. Speed-priority opt-in mode (ignore band ceiling) is a F-036d follow-up if anyone needs it. Smoke baseline diff: **no regressions**. Workspace clippy + tests clean.
