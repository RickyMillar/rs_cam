...
| [F-039](findings/F-039-constrained-max-feed-modulation.md) | Constrained-max feed modulation + transparency layers (per-toolpath modulation summary, per-move G-code comments, aggressiveness knob) — supersedes F-036's "target band-mid" default | sim + diagnostics | medium | L | open — design only |
