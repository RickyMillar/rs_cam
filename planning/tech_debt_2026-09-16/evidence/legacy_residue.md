# Legacy / compat / migration residue in production sources (526 lines, 107 files)
...
| 29 | `crates/rs_cam_core/src/compute/operation_configs.rs` |
| 25 | `crates/rs_cam_core/src/feeds/suggest.rs` |
| 21 | `crates/rs_cam_core/src/session/mod.rs` |
...
| 11 | `crates/rs_cam_core/src/session/project_file.rs` |
...
| 10 | `crates/rs_cam_core/src/gcode/mod.rs` |
...
| 7 | `crates/rs_cam_core/src/session/compute.rs` |
...
| 6 | `crates/rs_cam_cli/src/project.rs` |
...
| 5 | `crates/rs_cam_core/src/compute/tool_config.rs` |
...
| 2 | `crates/rs_cam_core/src/session/command.rs` |
| 2 | `crates/rs_cam_core/src/session/save.rs` |
...
| 2 | `crates/rs_cam_viz/src/mcp_server.rs` |
...
| 1 | `crates/rs_cam_core/src/simulation.rs` |
...
| 1 | `crates/rs_cam_viz/src/controller/results_parity_tests.rs` |
| 1 | `crates/rs_cam_viz/src/controller/events/mod.rs` |
| 1 | `crates/rs_cam_viz/src/controller/events/compute.rs` |
...
- `crates/rs_cam_core/src/dexel_mesh.rs:422` // 50-vertex / 288-index layout of the legacy heightmap. Assert
- `crates/rs_cam_core/src/dexel_mesh.rs:563` /// than the legacy fixed-index layout.
- `crates/rs_cam_core/src/dexel_mesh_mc.rs:3` //! Replaces the cell-centre heightmap mesh produced by the legacy
- `crates/rs_cam_core/src/dexel_mesh_mc.rs:741` // cuts match the legacy behaviour.
...
- `crates/rs_cam_core/src/spiral_finish.rs:642` /// the point-runs migration, non-contacted spiral samples were simply
- `crates/rs_cam_core/src/machine.rs:86` /// serde compatibility with existing project files and
- `crates/rs_cam_core/src/machine.rs:322` /// Parse a persisted preset key. Consumers: the viz LEGACY project
- `crates/rs_cam_core/src/machine.rs:343` /// (`from_key` above only reads legacy files); structural equality
- `crates/rs_cam_core/src/finish_surface_cache.rs:93` //! `cell_source`). `explicit(0.75)` and a `legacy_envelope_quarter` that
- `crates/rs_cam_core/src/finish_surface_cache.rs:502` let derived = FinishResolutionPolicy::legacy_envelope_quarter(&ball, 0.05);
- `crates/rs_cam_core/src/finish_surface_cache.rs:517` let floored = FinishResolutionPolicy::legacy_envelope_quarter(&fine, 0.5);
- `crates/rs_cam_core/src/unified_finish.rs:30` //! up). `claims: None` reproduces the pre-v3 op byte-for-byte: `decompose`
- `crates/rs_cam_core/src/unified_finish.rs:333` /// **Serde compatibility is load-bearing.** The two pre-A/M6 names are
- `crates/rs_cam_core/src/unified_finish.rs:555` /// reproduces the pre-v3 op exactly: no detector run, no crease claims, no
- `crates/rs_cam_core/src/unified_finish.rs:661` /// surface, same as the pre-v3 op. Creases still claim corridors.
- `crates/rs_cam_core/src/unified_finish.rs:771` /// the GUI span list, so this string is a compatibility surface.
- `crates/rs_cam_core/src/unified_finish.rs:922` /// on any plain ball — the migration moves tapered tools only.
- `crates/rs_cam_core/src/unified_finish.rs:1583` // byte-identical to the pre-v3 op (module doc).
- `crates/rs_cam_core/src/unified_finish.rs:2347` // material and the legacy surface-riding link is correct;
- `crates/rs_cam_core/src/unified_finish.rs:3539` /// table did, so the migration is provably value-preserving and the
- `crates/rs_cam_core/src/unified_finish.rs:4229` fn claims_off_matches_legacy_band_only_output() {
- `crates/rs_cam_core/src/collision.rs:169` /// Use 0.0 to check endpoints only (legacy behavior).
- `crates/rs_cam_core/src/collision.rs:290` /// `step_mm <= 0.01` checks the move endpoint only (legacy behavior).
...
- `crates/rs_cam_core/src/dressup.rs:1052` // LADDER OR PLUNGE, and never the legacy legs below. Both of the
- `crates/rs_cam_core/src/dressup.rs:1090` // The rapid keeps the air part at rapid speed, as the legacy
- `crates/rs_cam_core/src/dressup.rs:1195` // Legacy blind legs — honest only with no mesh surface to probe AND
- `crates/rs_cam_core/src/dressup.rs:1214` /// polyline round-trips to exactly the legacy moves. The first point of
- `crates/rs_cam_core/src/dressup.rs:3970` // legacy unconditional link.
- `crates/rs_cam_core/src/dressup.rs:3997` /// Entry safety with no surface probe — the legacy blind-leg
- `crates/rs_cam_core/src/dropcutter.rs:322` // compatibility — see the struct-level doc caveat: those two do not
...
- `crates/rs_cam_core/src/steep_shallow.rs:514` /// SteepShallow selects `FinishResolutionMode::LegacyEnvelopeQuarter` — the
- `crates/rs_cam_core/src/steep_shallow.rs:518` /// # Why this op STAYS on the legacy cell (PR-8c, written deferral)
- `crates/rs_cam_core/src/steep_shallow.rs:559` FinishResolutionPolicy::legacy_envelope_quarter(cutter, tolerance)
...
- `crates/rs_cam_core/src/machine_kinematics.rs:712` /// collects feed moves from legacy generators that never tagged
- `crates/rs_cam_core/src/machine_kinematics.rs:1642` fn legacy_kinematics_json_defaults_new_fields() {
- `crates/rs_cam_core/src/machine_kinematics.rs:1646` let legacy =
- `crates/rs_cam_core/src/machine_kinematics.rs:1648` let kin: MachineKinematics = serde_json::from_str(legacy).expect("legacy deserialize");
...
- `crates/rs_cam_core/src/pencil.rs:1935` // Wave D1: this legacy entry point predates the tip-float channel
- `crates/rs_cam_core/src/pencil.rs:2209` // longer read — it is deprecated and reported, see
...
- `crates/rs_cam_core/src/simulation.rs:1` //! Legacy simulation module — re-exports shared types for backward compatibility.
- `crates/rs_cam_core/src/monotone_cells.rs:109` /// compatibility). At 0° that fast path is honest — the rotation there IS
- `crates/rs_cam_core/src/machine_library.rs:8` //! pick up a change. There is no live `machine_ref` link (a legacy one is
- `crates/rs_cam_core/src/machine_library.rs:9` //! dropped on load and migrated into the inline copy).
- `crates/rs_cam_core/src/machine_library.rs:205` // to snapshot (legacy `machine_ref` is now dropped on load).
...
- `crates/rs_cam_core/src/scallop.rs:1829` /// Scallop selects `FinishResolutionMode::LegacyEnvelopeQuarter` — the same
- `crates/rs_cam_core/src/scallop.rs:1845` FinishResolutionPolicy::legacy_envelope_quarter(cutter, tolerance)
- `crates/rs_cam_core/src/scallop.rs:1945` /// `None` is the legacy relink, byte for byte: `reorder: false`,
- `crates/rs_cam_core/src/scallop.rs:2170` /// `link_stage: None` is the legacy intra-pass relink, byte for byte — which
- `crates/rs_cam_core/src/scallop.rs:2683` // legacy arm rejected 493 as `too_far` and ZERO on the surface or
- `crates/rs_cam_core/src/scallop.rs:2691` // test): the legacy literal below, byte for byte.
- `crates/rs_cam_core/src/scallop.rs:2699` let legacy = crate::surface_link::RelinkParams {
- `crates/rs_cam_core/src/scallop.rs:2735` None => (legacy, None),
...
- `crates/rs_cam_core/src/diagnostics/adapters/from_generation.rs:217` /// and the same rule already governs `zero_removal` and `deprecated_dial`.
- `crates/rs_cam_core/src/diagnostics/adapters/from_generation.rs:321` /// deprecated-dial rule guards against: claims run only on a `UnifiedFinish`
- `crates/rs_cam_core/src/diagnostics/adapters/from_generation.rs:466` /// `record_deprecated_dial` follows.
- `crates/rs_cam_core/src/diagnostics/adapters/from_generation.rs:536` fn deprecated_dial(toolpath_id: ToolpathId, stats: &ToolpathStats) -> Vec<Diagnostic> {
- `crates/rs_cam_core/src/diagnostics/adapters/from_generation.rs:538` let Some(f) = stats.deprecated_dial.as_deref() else {
- `crates/rs_cam_core/src/diagnostics/adapters/from_generation.rs:542` id: DiagnosticId::from(ids::CONFIG_DEPRECATED_DIAL),
...
- `crates/rs_cam_core/src/session/multitool.rs:1475` // Absent entries default to unified — byte-identical legacy plans.
...
- `crates/rs_cam_core/src/session/mod.rs:1200` /// The wire emits this under BOTH `truncated_core_mm2` and the legacy
- `crates/rs_cam_core/src/session/mod.rs:1375` /// Structured project-level verdict. Replaces the legacy single-line
- `crates/rs_cam_core/src/session/mod.rs:1377` /// (with the highest-severity verdict's headline) for backward compatibility
- `crates/rs_cam_core/src/session/mod.rs:1397` /// Legacy name, unchanged value: identical to
- `crates/rs_cam_core/src/session/mod.rs:1417` /// Legacy single-line verdict. Derived from the highest-severity entry
- `crates/rs_cam_core/src/session/mod.rs:2124` // Wave 16 / A6 compatibility: the SAME value under the pre-rename
- `crates/rs_cam_core/src/session/mod.rs:2129` // duplicate is deprecated and carries no independent meaning.
- `crates/rs_cam_core/src/session/mod.rs:2203` // LH-1: the legacy key keeps its (total-runtime) value for wire
- `crates/rs_cam_core/src/session/mod.rs:2204` // compatibility; the two named keys beside it say which denominator
- `crates/rs_cam_core/src/session/mod.rs:2334` _legacy_feeds_auto: None,
- `crates/rs_cam_core/src/session/mod.rs:2354` /// struct only so legacy projects still PARSE — it is no longer a
- `crates/rs_cam_core/src/session/mod.rs:2356` /// `legacy_machine_ref_dropped_on_session_load`).
- `crates/rs_cam_core/src/session/mod.rs:2358` fn legacy_machine_ref_field_still_parses_for_backcompat() {
- `crates/rs_cam_core/src/session/mod.rs:2389` /// Snapshot migration: loading a project that carries a legacy
- `crates/rs_cam_core/src/session/mod.rs:2393` fn legacy_machine_ref_dropped_on_session_load() {
- `crates/rs_cam_core/src/session/mod.rs:2400` name: "Legacy Ref".to_owned(),
- `crates/rs_cam_core/src/session/mod.rs:2413` let path = dir.join("legacy_ref.toml");
- `crates/rs_cam_core/src/session/mod.rs:2420` "legacy machine_ref must be dropped on load (snapshot model)"
- `crates/rs_cam_core/src/session/mod.rs:2493` /// accepts the former viz-legacy aliases — pre-T8 `"ball"` parsed
- `crates/rs_cam_core/src/session/mod.rs:2494` /// to EndMill here but BallNose in the viz legacy loader.
- `crates/rs_cam_core/src/session/mod.rs:2585` _legacy_feeds_auto: None,
- `crates/rs_cam_core/src/session/command.rs:646` "C01 deleted the legacy project reader, the only GUI caller",
- `crates/rs_cam_core/src/session/command.rs:658` "C01 deleted the legacy project reader; a load takes the name from the file",
- `crates/rs_cam_core/src/session/save.rs:63` /// Ported verbatim in behaviour from `rs_cam_viz`'s fallback loader
- `crates/rs_cam_core/src/session/save.rs:248` _legacy_feeds_auto: None,
...
- `crates/rs_cam_core/src/session/compute.rs:5211` // The legacy `air_cut_percentage` keeps its total-runtime value —
- `crates/rs_cam_core/src/session/compute.rs:5448` // Legacy single-line verdict for backward-compat callers: take the
- `crates/rs_cam_core/src/session/compute.rs:6982` // Legacy single-line verdict mirrors the top-severity headline.
- `crates/rs_cam_core/src/session/compute.rs:7719` deprecated_dial: None,
- `crates/rs_cam_core/src/session/project_file.rs:36` /// Legacy: top-level toolpaths (pre-setup format).
- `crates/rs_cam_core/src/session/project_file.rs:123` /// LEGACY. Machines now use snapshot semantics (the inline `machine`
- `crates/rs_cam_core/src/session/project_file.rs:510` /// Legacy field — older projects emit a `feeds_auto = {...}` block.
- `crates/rs_cam_core/src/session/project_file.rs:514` pub _legacy_feeds_auto: Option<toml::Value>,
- `crates/rs_cam_core/src/session/project_file.rs:568` /// legacy alias like `ball`) became an end mill with no trace.
- `crates/rs_cam_core/src/session/project_file.rs:699` // One-shot migration: projects saved before operation-specific dressup
- `crates/rs_cam_core/src/session/project_file.rs:710` "Normalized incompatible dressups on load"
- `crates/rs_cam_core/src/session/project_file.rs:1037` // Legacy: top-level toolpaths -> single default setup
- `crates/rs_cam_core/src/session/project_file.rs:1102` // `[job.machine]` copy is authoritative. A legacy `machine_ref` is no
- `crates/rs_cam_core/src/session/project_file.rs:1104` // migrated forward as-is). Re-save to persist the snapshot.
- `crates/rs_cam_core/src/session/project_file.rs:1107` "project '{}': legacy machine_ref {name:?} dropped — machines are now stored \
...
- `crates/rs_cam_core/src/adaptive3d/mod.rs:679` // but the unit-test fixtures here exercise the legacy code path
- `crates/rs_cam_core/src/adaptive3d/clearing.rs:1952` crate::adaptive::CleanupStrategy::Legacy
- `crates/rs_cam_core/src/adaptive3d/clearing.rs:2004` // For non-Legacy cleanup strategies, run the same cleanup
- `crates/rs_cam_core/src/adaptive3d/clearing.rs:2013` crate::adaptive::CleanupStrategy::Legacy => segs_2d_raw,
- `crates/rs_cam_core/src/adaptive3d/path.rs:1065` /// otherwise (caller falls back to the legacy retract path).
- `crates/rs_cam_core/src/adaptive3d/path.rs:1675` // F-038b: legacy `segments_to_toolpath` tests need a mesh + index +
- `crates/rs_cam_core/src/adaptive3d/path.rs:1689` fn legacy_test_mesh() -> (crate::mesh::TriangleMesh, crate::mesh::SpatialIndex) {
- `crates/rs_cam_core/src/adaptive3d/path.rs:1712` fn legacy_test_cutter() -> crate::tool::FlatEndmill {
- `crates/rs_cam_core/src/adaptive3d/path.rs:1768` // F-038b: legacy `segments_to_toolpath` unit tests expect the
- `crates/rs_cam_core/src/adaptive3d/path.rs:1808` let cutter = legacy_test_cutter();
- `crates/rs_cam_core/src/adaptive3d/path.rs:1892` let cutter = legacy_test_cutter();
- `crates/rs_cam_core/src/adaptive3d/path.rs:1958` let cutter = legacy_test_cutter();
- `crates/rs_cam_core/src/adaptive3d/path.rs:2115` let (mesh, si) = legacy_test_mesh();
- `crates/rs_cam_core/src/adaptive3d/path.rs:2116` let cutter = legacy_test_cutter();
...
- `crates/rs_cam_core/src/feeds/suggest.rs:2639` /// Pass 8: v1.3 combined-Suggest plunge-entry / DPP compatibility warning.
- `crates/rs_cam_core/src/feeds/suggest.rs:2947` /// compatibility warning. Returns `None` for non-Adaptive ops, for
- `crates/rs_cam_core/src/feeds/suggest.rs:3089` /// S1: with no scallop target the legacy formula-based stepover is
- `crates/rs_cam_core/src/feeds/suggest.rs:3094` fn scallop_height_none_preserves_legacy_ae() {
- `crates/rs_cam_core/src/feeds/suggest.rs:3098` "legacy DropCutter stepover on a 1 mm ball should stay small \
- `crates/rs_cam_core/src/feeds/suggest.rs:3427` /// legacy entry points it now backs — the funnel was extended, not
- `crates/rs_cam_core/src/feeds/suggest.rs:3430` fn apply_scope_matches_the_legacy_entry_points_exactly() {
- `crates/rs_cam_core/src/feeds/suggest.rs:3448` let mut legacy_op = base.clone();
- `crates/rs_cam_core/src/feeds/suggest.rs:3449` let mut legacy_prov = crate::feeds::FeedsProvenance::default();
- `crates/rs_cam_core/src/feeds/suggest.rs:3450` let legacy = match scope {
- `crates/rs_cam_core/src/feeds/suggest.rs:3455` legacy(
- `crates/rs_cam_core/src/feeds/suggest.rs:3456` &mut legacy_op,
- `crates/rs_cam_core/src/feeds/suggest.rs:3457` &mut legacy_prov,
- `crates/rs_cam_core/src/feeds/suggest.rs:3472` legacy_op.feed_rate(),
- `crates/rs_cam_core/src/feeds/suggest.rs:3477` legacy_op.plunge_rate(),
- `crates/rs_cam_core/src/feeds/suggest.rs:3482` legacy_op.spindle_rpm(),
- `crates/rs_cam_core/src/feeds/suggest.rs:3487` legacy_op.as_params().stepover(),
- `crates/rs_cam_core/src/feeds/suggest.rs:3492` legacy_op.as_params().depth_per_pass(),
- `crates/rs_cam_core/src/feeds/suggest.rs:4017` /// - DPP=9 mm, D=6 mm (pre-v1.1 motivating failure, ratio 1.5)
- `crates/rs_cam_core/src/feeds/suggest.rs:4026` (9.0, "pre-v1.1 case: DPP=9 mm on 6 mm tool (ratio 1.5)"),
...
- `crates/rs_cam_core/src/adaptive/mod.rs:264` CleanupStrategy::Legacy => segments,
- `crates/rs_cam_core/src/adaptive/mod.rs:330` cleanup_strategy: CleanupStrategy::Legacy,
...
- `crates/rs_cam_core/src/compute/catalog.rs:1516` /// geometrically incompatible with this op: compute strips them all
- `crates/rs_cam_core/src/compute/catalog.rs:1558` /// generation has been migrated onto the shared `GenerateFn`
- `crates/rs_cam_core/src/compute/catalog.rs:1561` /// in `execute_operation_annotated`. Migration state is pinned by
- `crates/rs_cam_core/src/compute/catalog.rs:1562` /// `generate_adapter_migration_is_an_explicit_per_op_decision`.
- `crates/rs_cam_core/src/compute/catalog.rs:1621` "enum:Legacy|ResidueMop|ContourParallelNarrow|ContourParallelHybrid",
- `crates/rs_cam_core/src/compute/catalog.rs:1828` // M8 (2026-09-03) — iso-field ring source; absent = legacy cascade.
- `crates/rs_cam_core/src/compute/catalog.rs:1853` // v3 S1/S2 claims pipeline: serde-defaulted for project-file back-compat
- `crates/rs_cam_core/src/compute/catalog.rs:1866` // territory_clip` doc) — same serde-defaulted back-compat treatment.
- `crates/rs_cam_core/src/compute/catalog.rs:1869` // back-compat treatment again.
- `crates/rs_cam_core/src/compute/catalog.rs:1885` // §9/§11 link caps, both serde-defaulted for back-compat:
- `crates/rs_cam_core/src/compute/catalog.rs:2285` "Incompatible with 3D Finish: each raster segment's ramp entry would carve a diagonal trench across the stock.",
- `crates/rs_cam_core/src/compute/catalog.rs:2407` "Incompatible with Unified Finish: ramp/lead/link dressups would carve diagonal trenches across the mesh surface; the op emits its own surfa
- `crates/rs_cam_core/src/compute/catalog.rs:2525` "Incompatible with Project Curve: each ring would get a phantom diagonal cut.",
- `crates/rs_cam_core/src/compute/catalog.rs:2610` // does. `None` keeps the legacy `ae_factor × diameter` stepover.
- `crates/rs_cam_core/src/compute/catalog.rs:2825` /// Phase 5 (T11): GenerateFn migration is a per-family DECISION,
- `crates/rs_cam_core/src/compute/catalog.rs:2826` /// not drift. Each op is either migrated (registry adapter — the
- `crates/rs_cam_core/src/compute/catalog.rs:2831` fn generate_adapter_migration_is_an_explicit_per_op_decision() {
...
- `crates/rs_cam_core/src/compute/tool_config.rs:73` /// project loader (`ballnose`, wildcard→EndMill), viz legacy loader
- `crates/rs_cam_core/src/compute/tool_config.rs:78` /// `"ball"` parsed to `BallNose` in the viz legacy loader but
- `crates/rs_cam_core/src/compute/tool_config.rs:87` // "flat" is the legacy save format's EndMill token
- `crates/rs_cam_core/src/compute/tool_config.rs:943` /// wire format, MCP error messages, and the viz legacy tool-type
- `crates/rs_cam_core/src/compute/tool_config.rs:977` /// historical viz-legacy aliases all parse; case is folded; unknown
...
- `crates/rs_cam_core/src/compute/operation_configs.rs:602` /// Deprecated + inert sidewall leave allowance. NOT honored by the
- `crates/rs_cam_core/src/compute/operation_configs.rs:876` /// [`crate::compute::config::DeprecatedDialFinding`] →
- `crates/rs_cam_core/src/compute/operation_configs.rs:877` /// `diagnostics::ids::CONFIG_DEPRECATED_DIAL`, so the operator is told
- `crates/rs_cam_core/src/compute/operation_configs.rs:882` /// rest reference (all three detectors). `None` = legacy nominal-diameter
- `crates/rs_cam_core/src/compute/operation_configs.rs:974` /// §M7–M8). Default OFF: an absent key loads the legacy cascade
- `crates/rs_cam_core/src/compute/operation_configs.rs:1073` /// carved). `false` reproduces the pre-v3 op exactly — no detector
- `crates/rs_cam_core/src/compute/operation_configs.rs:1371` /// absent key now loads `true`: a legacy project file gets the cell
- `crates/rs_cam_core/src/compute/operation_configs.rs:1373` /// per-operation opt-out, and the X5 back-compat test pins both directions.
- `crates/rs_cam_core/src/compute/operation_configs.rs:2481` /// The A/M6 deserialization-compatibility matrix, all three cells
- `crates/rs_cam_core/src/compute/operation_configs.rs:2492` /// the letter, and the wire names are the pre-A/M6 ones so no migration
- `crates/rs_cam_core/src/compute/operation_configs.rs:2497` fn unified_finish_claims_reference_serde_round_trip_and_backcompat() {
- `crates/rs_cam_core/src/compute/operation_configs.rs:2550` // Cell 3: legacy payload predating `claims_reference` and its S1/S2
- `crates/rs_cam_core/src/compute/operation_configs.rs:2553` let legacy = r#"{
- `crates/rs_cam_core/src/compute/operation_configs.rs:2566` let cfg: UnifiedFinishConfig = serde_json::from_str(legacy).unwrap();
- `crates/rs_cam_core/src/compute/operation_configs.rs:2569` // legacy project only sees the new default act if it also turned
- `crates/rs_cam_core/src/compute/operation_configs.rs:2572` // S4 (`territory_clip`) postdates this legacy payload too — must
- `crates/rs_cam_core/src/compute/operation_configs.rs:2573` // default off, same backcompat contract as its S1/S2 siblings.
- `crates/rs_cam_core/src/compute/operation_configs.rs:2578` // ruling"); the default is now ON. An absent key loads ON: a legacy
- `crates/rs_cam_core/src/compute/operation_configs.rs:2584` let pinned = legacy.replace(
- `crates/rs_cam_core/src/compute/operation_configs.rs:2662` /// Drill selection fields must round-trip, and legacy TOML/JSON that
- `crates/rs_cam_core/src/compute/operation_configs.rs:2663` /// predates them must deserialize to the legacy "all centroids" behaviour
- `crates/rs_cam_core/src/compute/operation_configs.rs:2666` fn drill_selection_serde_round_trip_and_backcompat() {
- `crates/rs_cam_core/src/compute/operation_configs.rs:2667` // Legacy payload with neither selection key present.
- `crates/rs_cam_core/src/compute/operation_configs.rs:2668` let legacy = r#"{
- `crates/rs_cam_core/src/compute/operation_configs.rs:2677` let cfg: DrillConfig = serde_json::from_str(legacy).unwrap();
- `crates/rs_cam_core/src/compute/operation_configs.rs:2678` assert_eq!(cfg.selected_holes, None, "legacy => all targets (None)");
- `crates/rs_cam_core/src/compute/config.rs:153` /// The old spelling survives ONLY as a legacy JSON key emitted beside
- `crates/rs_cam_core/src/compute/config.rs:282` /// a measurement, it is a compatibility notice: the field is still
- `crates/rs_cam_core/src/compute/config.rs:291` pub deprecated_dial: Option<Box<DeprecatedDialFinding>>,
- `crates/rs_cam_core/src/compute/config.rs:443` /// `None` = **nothing inert is set**. Like [`Self::deprecated_dial`] this
- `crates/rs_cam_core/src/compute/config.rs:1002` pub struct DeprecatedDialFinding {
- `crates/rs_cam_core/src/compute/config.rs:1021` /// value AND the retired one is what makes the migration checkable on a real
- `crates/rs_cam_core/src/compute/config.rs:1487` /// `post.safe_z` is a legacy 2D-friendly default (often 10mm above work Z=0),
- `crates/rs_cam_core/src/compute/config.rs:1636` /// keep working for callers that haven't migrated; combining with the
- `crates/rs_cam_core/src/compute/config.rs:2084` /// disabling any dressup that's geometrically incompatible with how the
- `crates/rs_cam_core/src/compute/config.rs:2085` /// operation emits toolpaths. Intended as a one-shot migration on
- `crates/rs_cam_core/src/compute/config.rs:2173` // Backwards-compat for callers that build `HeightContext::simple` /
- `crates/rs_cam_core/src/compute/config.rs:2283` fn backward_compat_old_json() {
...
- `crates/rs_cam_core/src/compute/execute.rs:2162` /// stage is then byte-identical to the legacy surface-riding link, which is
- `crates/rs_cam_core/src/compute/execute.rs:2400` record_deprecated_dial(
- `crates/rs_cam_core/src/compute/execute.rs:2402` crate::compute::config::DeprecatedDialFinding {
- `crates/rs_cam_core/src/compute/execute.rs:2507` // legacy relink at the same `intra_pass_hookup_mm`, byte for byte.
- `crates/rs_cam_core/src/compute/execute.rs:3399` /// here and discards the spans for backwards compatibility.
- `crates/rs_cam_core/src/compute/execute.rs:3494` /// against. `None` is a byte-identical no-op (legacy distance-only
- `crates/rs_cam_core/src/compute/execute.rs:3520` // legacy distance-only hookup decision.
- `crates/rs_cam_core/src/compute/execute.rs:3536` // exhaustive match below remains the fallback for unmigrated
- `crates/rs_cam_core/src/compute/execute.rs:3538` // compile until it has an arm — migrated arms delegate to the SAME
- `crates/rs_cam_core/src/compute/execute.rs:3578` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3581` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3584` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3587` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3590` // Migrated to the registry GenerateFn (T11); arms kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3594` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3597` // Migrated to the registry GenerateFn (T11); arms kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3601` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3604` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3607` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3612` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3615` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3618` // Migrated to the registry GenerateFn (T11); arm kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3621` // Migrated to the registry GenerateFn (T11); arms kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3628` // Migrated to the registry GenerateFn (T11); arms kept for the
- `crates/rs_cam_core/src/compute/execute.rs:3633` // Migrated to the registry GenerateFn (T11); arm kept for the
...
- `crates/rs_cam_core/src/gcode/mod.rs:394` /// benefit of the doubt — backward-compat with older traces — and
- `crates/rs_cam_core/src/gcode/mod.rs:402` // the legacy behaviour of treating it as fresh; the user can
- `crates/rs_cam_core/src/gcode/mod.rs:429` // missing entry as a config match for backward-compat.
- `crates/rs_cam_core/src/gcode/mod.rs:1133` fn checked_phased_export_is_byte_identical_to_legacy_emitter() {
- `crates/rs_cam_core/src/gcode/mod.rs:1173` let legacy = emit_gcode_phased(&phases, post::grbl());
- `crates/rs_cam_core/src/gcode/mod.rs:1182` assert_eq!(checked, legacy);
- `crates/rs_cam_core/src/gcode/mod.rs:1186` fn checked_multi_setup_export_is_byte_identical_to_legacy_emitter() {
- `crates/rs_cam_core/src/gcode/mod.rs:1234` let legacy = emit_gcode_multi_setup(&setups, post::grbl(), 15.0);
- `crates/rs_cam_core/src/gcode/mod.rs:1244` assert_eq!(checked, legacy);
...
- `crates/rs_cam_viz/src/mcp_server.rs:501` description = "Project diagnostics. Read the `triage` block FIRST — it is the bounded, severity-ordered answer to 'what should I act on?' (s
...
- `crates/rs_cam_viz/src/state/viewport.rs:205` /// rename carries no wire compatibility.
...
- `crates/rs_cam_viz/src/ui/properties/mod.rs:3709` /// compatibility — G-BOUNDARYINHERIT) and `planner_origin` (the
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5567` // Some dressups are geometrically incompatible with specific operations
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5575` let op_incompatible_msg: Option<&str> = dressup_policy.strip_all_reason;
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5582` // keep using op_incompatible_msg).
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5583` let entry_disabled_msg: Option<&str> = op_incompatible_msg.or_else(|| {
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5666` ui.add_enabled_ui(op_incompatible_msg.is_none(), |ui| {
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5670` if let Some(msg) = op_incompatible_msg {
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5674` if cfg.lead_in_out && op_incompatible_msg.is_none() {
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5702` ui.add_enabled_ui(op_incompatible_msg.is_none(), |ui| {
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5706` if let Some(msg) = op_incompatible_msg {
- `crates/rs_cam_viz/src/ui/properties/mod.rs:5710` if cfg.link_moves && op_incompatible_msg.is_none() {
...
- `crates/rs_cam_viz/src/controller/results_parity_tests.rs:307` "the deprecated A6 duplicate key carries the same value, as on the CLI wire"
- `crates/rs_cam_viz/src/controller/events/mod.rs:312` // Empty selection reverts to the legacy default (None) so a
- `crates/rs_cam_viz/src/controller/events/compute.rs:2098` // is which. The legacy `air_cut_percentage` key keeps its
...
- `crates/rs_cam_cli/src/project.rs:76` /// A6 compatibility duplicate: the SAME value as
- `crates/rs_cam_cli/src/project.rs:78` /// existing scripts reading this report keep working. Deprecated; it
- `crates/rs_cam_cli/src/project.rs:192` /// Legacy key, unchanged value: identical to
- `crates/rs_cam_cli/src/project.rs:982` {} issue runs (COALESCED, the legacy \"issue_count\") | {} hotspots",
- `crates/rs_cam_cli/src/project.rs:1032` /// This record's key names are a compatibility surface — the doc comment
- `crates/rs_cam_cli/src/project.rs:1097` // A6: the compatibility duplicate obeys the same contract — an
