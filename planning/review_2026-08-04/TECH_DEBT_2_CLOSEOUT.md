...
| id | item | owner | re-open condition |
|---|---|---|---|
| **G-LV.1** | **The GUI frame loop can park, and a parked loop is indistinguishable from success.** Live 2026-08-07, release build at `73e2376`, wanaka, `simulation_resolution_mm 0.1`: a `generate_all` fixpoint run stalled indefinitely between rounds once the GUI window stopped repainting (hidden/locked). The lane finished `3D Rough 6` and went idle, the simulate-round handoff never ran, and `generation_status` answered *"idle: no toolpath generation in flight"* — true, and read as completion. Evidence: a server-thread `generation_status` produced **0 CPU ticks in the following 8 s**; a frame-loop `list_toolpaths` produced a multi-core burst then silence; `list_toolpaths` snapshot ages grew **155 s → 268 s**. **Diagnosed, reported, and NOT removable from inside the app**: on Wayland a repaint request reaches `Window::request_redraw`, and winit 0.30.13 refuses to emit `RedrawRequested` while the surface awaits a compositor frame callback (`wayland/event_loop/mod.rs`, `FrameCallbackState::Requested`), which a hidden/occluded/locked surface never gets; eframe 0.34.3's own rescue is gated on `is_invisible_or_minimized`, and winit's Wayland `is_visible()`/`is_minimized()` both return `None`. **Shipped this fix**: every enqueue wakes the GUI *and* records that work is stranded (`FrameLoopBeat`); the fixpoint's pending work holds a standing repaint request; `generation_status` / `cancel_generation` / the snapshot fallback all report `frame_loop` and refuse to let an idle lane read as done; `--mcp` on Wayland warns at startup | the viz/MCP lane | **The remaining hazard is the platform park itself, not its reporting.** Re-open with a *mechanism*, not another `request_repaint`: (a) run the GUI under `WINIT_UNIX_BACKEND=x11`, where redraws are client-driven and the park cannot occur — cheapest, already the documented remedy, needs a render check that XWayland scaling is acceptable; or (b) move the compute drain and the fixpoint handoff off the paint path entirely, which is an architectural change (`Controller` is main-thread-owned and not `Send`-shared) and needs its own package. Do **not** re-open on "add a repaint request somewhere else": three already exist on this path and the live stall happened with all three in place |
...
| **G-LV.2** | **An unfiltered `get_cut_trace` kills the GUI.** Live 2026-08-07, release `d820226`, after a full-project 0.1 mm sim (1,588,883 samples): `get_cut_trace` with no toolpath filter closed the MCP connection mid-response and the GUI process died; system RAM was fine (~24 GB available). The filtered call succeeded seconds earlier. Two-step repro. Cause unattributed (no journal/core access) — serialization OOM is a hypothesis, not a finding. This is C25's re-open condition arriving as an incident: the parameterised-read path, measured against a live project, does not merely stall — it takes the process with it. Same area, same commit: the `toolpath_id` filter matches drill summaries/samples by **id** while its doc says "by index" (index 0 returns empty, id 14 answers) | the viz/MCP lane (with C25) | none needed — the repro is two calls. Bound or paginate the unfiltered response (C25's snapshot/continuation design is the shape), fix the filter doc/behaviour mismatch while in there, and capture the crash cause under a debugger before trusting the OOM hypothesis |
| **G-CHIP-ULP** | **The chipload gate's boundary comparison flips a verdict on 1 ulp, and the floor==ceiling collision parks every affected op exactly on that boundary.** Live on wanaka's Unified Finish (tip-matched 0.1 mm): observed `0.012781077012073162` vs band max `0.01278107701207316` → `Exceeds(High)`; the printed arithmetic reads "0.012781 vs 0.012781 → Exceeds". Mechanism, read off the GUI's own (correct) warning: the derated band sits wholly below the 0.025 rubbing floor, `effective_rubbing_floor == band ceiling`, modulation drives the feed onto the ceiling, the gate compares with float noise deciding the side. Riders recorded live: the band is resolution-dependent (queried effective Ø 1.308 → 1.372 between 1.0/0.1 mm cells, so coarse read `Within` at 97.8 % and fine read `Exceeds`); `modulation_summary` does not list `chipload_max` as binding while parking the peak on the band max; and boundary semantics disagree across gates (drill `plunge_feed` observed==lower bound ⇒ `Within`, chipload observed==upper bound ⇒ `Exceeds`) | the feeds lane | decide the boundary contract once (inclusive-with-epsilon at both ends, stated on `ChipBounds`), make the floor-clamped case say *clamped* rather than *exceeds* on the verdict surface (the hints already do), and account the floor clamp as a binding constraint in `modulation_summary`. A fixture is trivial: any band whose derated max < 0.025. **AMENDED 2026-08-13 per Checkpoint K (A-6 measurement, A-7 fix):** the fourth rider recorded above — "boundary semantics disagree across gates" — is **factually wrong**. A-6 drove all nine shipped gate/side pairs at their exact bounds (`tests/chipload_boundary_g_chip_ulp.rs::cross_gate_boundary_semantics_at_exact_equality`): **every gate is `Within` at exact equality**, including the drill `plunge_feed` and chipload pairs this row contrasts. The live `Exceeds` was rider 2 — the observation sat **1 ulp above** the bound, not equal to it — so the defect is **reconstruction**, not semantics: Suggest multiplies (`fpt × rpm × flutes`) and the gate divides, and that round trip lands strictly above the bound in **6–8 %** of a 1 164-point (rpm, flutes) grid. What genuinely differs across gates is (a) the drill gates carried **no epsilon dial at all** while the three milling gates each carried one defaulting to zero, and (b) only the chipload observation reaches its bound through a multiply/divide round trip. Also amended: the third rider is **correct and stands**, and Checkpoint K ruled it fixed on the *recipe* record (`CommandedStage::clamped_to`, option d2) rather than in `modulation_summary`'s vendor vocabulary (option d1) — the clamp is a whole-recipe decision the modulator never evaluates. **FIXED** at A-7: `tool_load::boundary` (`BOUNDARY_EPSILON_REL = 8 × f64::EPSILON`, relative, on every gate comparison in the crate), `ChiploadVerdict::Within::ceiling_advisory` (c2), `CommandedStage::clamped_to` (d2). The reproduction moved from **Exceeds at 12 of 181 RPM values** to **0 of 181**, with a genuine 5 %-over feed still tripping at all 181. Riders 1 and 2b are **unchanged and still true**: the floor still collapses onto the ceiling (that 2026-08-06 ruling was correct), and the band still moves **14.16 %** across a DOC sweep because peak axial DOC is a dexel measurement — record the simulation cell beside every chipload verdict. |
| **G-IPE-PLUNGE** | **NEW 2026-08-13 (Checkpoint K (e), A-7).** The literature-matrix cell `flat_3mm_pocket_ipe_extreme` reports **plunge/feed −61.1 %** below its expected fraction: the engine commands 0.1167 of the cutting feed where the cell's band is 0.30–0.50 (sources `shapeoko_wiki`, `vectric_default`). Surfaced while diagnosing G-LIT-IPE and **explicitly not a chipload question** — it is untouched by (e2)'s anti-test re-expression, by the two chipload scaling laws, and by the cell's separate literature-vs-LUT band conflict. It was previously invisible because the cell's `critical` verdict came from a redundant absolute anti-test; with that corrected, this is one of three honest `Outside` rows the cell now reports at moderate. **Not diagnosed**: whether the engine's plunge-to-feed ratio is wrong for micro tools in extreme-density hardwood, whether the cell's 0.30–0.50 band transfers to Ø3, or whether Suggest's Step-9c drill/plunge clamp interacts here | the feeds lane | measure the plunge-to-feed ratio the engine commands across the matrix's hardwood cells before touching either side — one cell is not a calibration. If the ratio is systematically low only on sub-Ø3 tools, that is a defect; if it is uniform, the cell's band is the thing to re-verify (fold into S-2's `/refresh-lit-matrix` pass, which already owns this cell) |
| **G-BYTE** | **Same-parameter regeneration of a rest-fed finish op is not byte-stable across sim events.** Item 11's 0.0-reset export vs baseline: 135 change-hunks — ~140 `G0` approach heights moved 0.05–0.15 mm and 12 of 180,118 cutting lines dropped (0.007 %); cutting Zs identical where present (the `stock_to_leave` dial itself round-trips exactly; the three-band +0.500 mm shift was exact on every narrated level). Signature indicts the in-process stock snapshot consumed at generation (fixpoint round sim vs later explicit full sim), i.e. the comparison violated §6.1 rule 6, not necessarily the generator — but nothing currently *records* which sim event a generation consumed, so the two causes cannot be told apart after the fact | the finishing/sim lane | the controlled A/B: freeze one machined-stock snapshot, generate the same op twice against it, byte-compare. If identical, the fix is provenance (stamp the consumed snapshot id into `ToolpathStats` so rule-6 violations are detectable); if not, it is generator nondeterminism and becomes a defect row |
...
| **F-LUT2** | Suggest and the gate still use **different LUT lookup entry points** (`find_best_row_for_geometry` vs `find_best_chip_envelope_row`) | Checkpoint B item 5 | measure the row-selection delta first (structural, number-preserving); only then decide the switch, which is number-moving. **AMENDED 2026-08-13 per Checkpoint K (A-6 census, A-7 fix): this row named the wrong axis.** The entry-point pair is the *smaller* half and is not the consequential divergence: over 18 144 swept queries the two resolvers return the same row **4 350** times, differ **141** times (0.78 %, three of 48 cells), and produce two different *bands* **zero** times — every divergence is one class, "Suggest matched an RPM-only anchor row and fell back to the formula chipload". The consequential divergence is the **query**, not the resolver: the gate side routes `Adaptive3d` → `Pocket` and `ProjectCurve` → `(Parallel|Contour, Finish)` through `routed_lookup_family`, and the Suggest side did not. On that axis **489 of 3 024** pairs resolved to different rows (band-max ratio ×0.16 … ×6.17, median ×0.98) plus **756** gate refusals of which **378** left Suggest holding a banded recommendation where the gate declined to judge at all. A-5's 1.273× band divergence is re-attributed to this reroute, proven two ways. Ruled a3 + a4: the two entry points **stay**, with declared purposes and a typed disclosure (`FeedsWarning::VendorRowPublishesNoChipload`) on the 141; and the routing is hoisted into one `vendor_normalize::lut_query_for` both consumers call. |
...
Fifteen conditions. **11 MET · 4 PARTIAL · 0 UNMET** (updated 2026-08-08
after the Checkpoint G live validation — see the W10-LV entry in
`ORCHESTRATION_LOG.md`; the original close-out shipped at 10 · 4 · 1 with
condition 14 UNMET). Four are PARTIAL and each says exactly what is
missing and where it is ledgered.
...
| 5 | a cascade/library failure cannot silently masquerade as a successful collapsed/empty operation | **PARTIAL — met in debug, unmeasured in release** | `offset_polygon_reported` separates `Collapsed` / `RejectedInput` / `LibraryFailure`; the boundary layer **refuses** on a library failure and passes through only on a genuine collapse, with a typed finding naming the dropped containment; `offset_library_failures` and `boundary_clip_dropped` publish it. But two of the three panic classes are `debug_assert!`s in transitive dependencies, so **in release the failure is not detected at all** and the library proceeds on unvalidated input. Accepted and documented at Checkpoint C Q4; **measured 2026-08-08** (W10-LV, §6 steps 14–17): the release arms print their branch honestly, the captured R1 asset reaches cavalier **unrepaired** in release and returns ok (6 plines) where debug refuses — the divergence is now a measured fact, per class: slice stitching exercised, spatial-index and NaN classes NOT EXERCISED. The verdict stays PARTIAL because the condition itself remains false in release for the debug_assert classes; what changed is that it is now measured rather than assumed |
...
| 14 | every behavioural output/default/operator-number change passed its checkpoint, focused gates, documented re-pins, and **final read-only live validation** | **MET** (2026-08-08) | The first three clauses were met at close-out: eight checkpoints ruled and executed; every behavioural change red-first with the parent failure quoted; every fingerprint move carrying old/new, mechanism, exact build and a consumer census. The fourth clause ran 2026-08-07/08 as the W10-LV session (`ORCHESTRATION_LOG.md`): all 24 items discharged with honest verdicts — the shipped behaviours held (drill exact-match to G-code, triage parity, measurability abstention, three-band stock_to_leave, P-1 persistence, surface-vs-mesh 0 over-cut), and the session's findings are new ledger rows (**G-LV.2**, **G-CHIP-ULP**, **G-BYTE**), not regressions of anything this programme shipped |
...
condition as written. The fourth (release behaviour) was PARTIAL because
the measurement was scheduled and not taken; as of 2026-08-08 it is taken
(W10-LV) and the row stays PARTIAL for the condition, not the evidence.

*(Historical note: this summary originally ended with one condition UNMET
awaiting operator authorization of the live validation. The operator
authorized it 2026-08-06; it ran 2026-08-07/08; condition 14 is MET. The
live results are in the W10-LV entry of `ORCHESTRATION_LOG.md` and the
three new findings are §4.2 rows G-LV.2, G-CHIP-ULP and G-BYTE.)*
