...
**Last touched:** 2026-05-20 — Step 5 J landed: marching-cubes (height-field reduction) replaces the cell-centre heightmap mesh in `z_grid_to_solid_mesh`. New `crates/rs_cam_core/src/dexel_mesh_mc.rs` module implements the corner-bilinear extraction; legacy heightmap retained as private `z_grid_to_solid_mesh_heightmap` for reference. Per §6.J revision, the SDF `sdf = interp_ray_top − z` is linear in z, so MC's iso reduces to the bilinearly-averaged height field — we emit it directly rather than running a voxel sweep. F.a's coverage-weighted `ray_top` from Step 4 feeds the bilinear average, giving sub-cell-accurate wall positions at F.a boundary cells. Closure (bottom face, perimeter skirt, hole walls, multi-segment cavity surfaces) is emitted explicitly. 7 legacy `dexel_mesh` tests rewritten from vertex-layout assertions to topology / point-cloud queries. Roadmap closed: all 5 steps ☑.
