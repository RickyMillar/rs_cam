...
| Step 2 — D + H (Engagement vector + per-kinematics summary) | ☑ done | `d3fd473` (tail PR — `radial_engagement` deletion — landed 2026-05-20) | `Engagement` struct + `EngagementDirection` enum on `SimulationCutSample`; legacy `radial_engagement` scalar **removed** in §10.3 tail PR after one release deprecation window — `SIMULATION_CUT_TRACE_SCHEMA_VERSION` bumped 3 → 4; all consumers read `sample.engagement.radial_woc_fraction`; `Engagement::with_radial_woc(r)` helper added to minimise fixture churn. `KinematicsSummary` block on `SimulationCutSummary` + `SimulationToolpathCutSummary`; `SummaryAccumulator.per_kinematics` switched from BTreeMap to `[KinematicsAccumulator; 5]` after the BTreeMap version regressed aggregation by 50% on 250K samples. MCP per-span + per-depth-pass JSON surfaces gain `per_kinematics` block. Air-cut / low-engagement / average-engagement docstrings updated to name the radial-WOC axis. CLAUDE.md "cylinder-volume engagement" caveat softened to point at `engagement.radial_woc_fraction` + per-kinematics block. |
| Step 3 — E (DrillOp first-class) | ☑ done | PR1: `5d32ae1` + `aff7738`; PR2: `47b5864` | **Path B confirmed (2026-05-19).** PR1 lands data model, dual-representation invariant, analytical stock removal, and analytic mesh emission. PR2 adds `DrillSample` stream (`drill_metrics.rs`), `DrillToolpathSummary`, drill-specific gates `DrillGatesVerdict { chip_welding, peck_adequacy, plunge_feed }` on `ToolpathLoadVerdict.drill_gates`, narrate-output enrichment, and an in-repo `drill_metrics_pr2.rs` integration test substituting for WANAKA TP0/TP2 revalidation (the WANAKA TOML lives outside the repo). |
| Step 4 — F.a (sub-cell stamping) | ☑ done | `be0dcbf` + `2770089` + `d4ef955` | All four §6.F gaps closed: degenerate-branch volume scales by coverage; new `ray_blend_above`/`ray_blend_below` primitives; bounding-box scan extended past `r` by the sub-sample half-extent (cs·√2·3/8) with `lut_h_with_edge_fallback`; per-cell `coverage_max: Vec<f32>` sidecar on `DexelGrid`. Sub-cell coverage via 4×4 sub-sampling with sub-sample-geometry fast paths. Perp-extent metric gated on `coverage ≥ 0.95` to prevent F.a residuals from inflating engagement on repeated passes. Planner-↔-sim parity threshold bumped from 1 % → 10 % (multiplicative blend doesn't compose perfectly under subsegmentation, per §6.F). New `sub_cell_stamping_fa.rs` integration tests lock in analytical disk-area / stadium-area volume invariants. WANAKA Back Rough revalidation lands in `wanaka_step4_fa_revalidation.rs`. Bench A/B: whole-system `simulate_toolpath_metrics/500moves` lands at **+37 %** (≈ §6.F's predicted 1.4×); cumulative Step 0–4 ≈ +40 %, within the §10.5 50 % ceiling. |
| Step 5 — J (marching cubes) | ☑ done | `81cd86e` + `c95ca19` | Vanilla MC + bilinear-SDF reduction. `z_grid_to_solid_mesh` now delegates to `dexel_mesh_mc::z_grid_marching_cubes`: corner-bilinear `ray_top` / `ray_bottom` height fields (the SDF `interp_ray_top − z` is linear in z, so the iso reduces to a height surface) + explicit perimeter skirt, hole walls, and per-gap cavity fallback. Live-preview path (`dexel_stock_to_entry_surface_mesh`) unchanged. Side-grid path dropped from scope (4-axis epic). All 1507 lib tests + 6 new Step 5 regression tests in `step5_marching_cubes.rs` pass (watertightness, vertex z-envelope, MC-vs-preview triangle ratio ≤ 10×, pocket floor/rim presence, drill-CSG seam ≤ 1.5× cell_size). WANAKA mesh revalidation in `wanaka_step5_mc_revalidation.rs`. |

**Last touched:** 2026-05-20 — Step 2 §10.3 tail PR landed: `SimulationCutSample.radial_engagement: f64` scalar deleted (the field was carried as a `#[deprecated]`-doc'd derived view since Step 2). All ~30 reads switched to `engagement.radial_woc_fraction`; schema version bumped 3 → 4; `Engagement::with_radial_woc(r)` helper added; `SimulationCutIssue.{radial_engagement, min_radial_engagement}` snapshot fields retained (they're issue-payload snapshots, distinct from the deleted sample-level scalar). Roadmap including tracked follow-ups now fully closed. /verify clean: 1676 tests passed, 0 failed, 122 ignored (pre-existing). Earlier same-day — Step 5 J landed: marching-cubes (height-field reduction) replaces the cell-centre heightmap mesh in `z_grid_to_solid_mesh`. New `crates/rs_cam_core/src/dexel_mesh_mc.rs` module implements the corner-bilinear extraction; legacy heightmap retained as private `z_grid_to_solid_mesh_heightmap` for reference (deleted 2026-09-16, duplicate sweep C27: no caller, no test). Per §6.J revision, the SDF `sdf = interp_ray_top − z` is linear in z, so MC's iso reduces to the bilinearly-averaged height field — we emit it directly rather than running a voxel sweep. F.a's coverage-weighted `ray_top` from Step 4 feeds the bilinear average, giving sub-cell-accurate wall positions at F.a boundary cells. Closure (bottom face, perimeter skirt, hole walls, multi-segment cavity surfaces) is emitted explicitly. 7 legacy `dexel_mesh` tests rewritten from vertex-layout assertions to topology / point-cloud queries. Roadmap closed: all 5 steps ☑.
