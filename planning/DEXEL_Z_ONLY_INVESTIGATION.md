...
**Last touched:** 2026-05-19 — Step 4 F.a landed: area-weighted sub-cell coverage replaces binary cell-center stamping in `stamp_point_on_grid` / `stamp_segment_on_grid` / `stamp_segment_with_metrics`. New `ray_blend_above` / `ray_blend_below` primitives in `dexel.rs` shrink the above-surface portion of each ray segment by a fraction `f ∈ [0, 1]`. Per-cell `coverage_max: Vec<f32>` sidecar on `DexelGrid` (forward-compat hook for F.b). All four §6.F gaps closed. Two parity-test thresholds and one full-slot arc-engagement tolerance relaxed with documented rationale tied to F.a's expected drift. Six new F.a regression tests (`sub_cell_stamping_fa.rs`) lock in analytical disk-area / stadium-area volume invariants; WANAKA Back Rough revalidation in `wanaka_step4_fa_revalidation.rs`. Step 4 closed; Step 5 (marching cubes) is next.
