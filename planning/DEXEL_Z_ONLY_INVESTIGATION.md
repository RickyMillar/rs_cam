...
| Step 4 — F.a (sub-cell stamping) | ☑ done | `be0dcbf` + `2770089` | All four §6.F gaps closed: degenerate-branch volume scales by coverage; new `ray_blend_above`/`ray_blend_below` primitives; bounding-box scan extended to `r + cs·√2` with `lut_h_with_edge_fallback`; per-cell `coverage_max: Vec<f32>` sidecar on `DexelGrid`. Sub-cell coverage via 4×4 sub-sampling with fast-path corner tests. Perp-extent metric gated on `coverage ≥ 0.95` to prevent F.a residuals from inflating engagement on repeated passes. Planner-↔-sim parity threshold bumped from 1 % → 10 % (multiplicative blend doesn't compose perfectly under subsegmentation, per §6.F). New `sub_cell_stamping_fa.rs` integration tests lock in analytical disk-area / stadium-area volume invariants. WANAKA Back Rough revalidation lands in `wanaka_step4_fa_revalidation.rs`. |
