...
| Step 5 — J (marching cubes) | ☑ done | `81cd86e` | Vanilla MC + bilinear-SDF reduction. `z_grid_to_solid_mesh` now delegates to `dexel_mesh_mc::z_grid_marching_cubes`: corner-bilinear `ray_top` / `ray_bottom` height fields (the SDF `interp_ray_top − z` is linear in z, so the iso reduces to a height surface) + explicit perimeter skirt, hole walls, and per-gap cavity fallback. Live-preview path (`dexel_stock_to_entry_surface_mesh`) unchanged. Side-grid path dropped from scope (4-axis epic). All 1507 lib tests + 6 new Step 5 regression tests in `step5_marching_cubes.rs` pass (watertightness, vertex z-envelope, MC-vs-preview triangle ratio ≤ 10×, pocket floor/rim presence, drill-CSG seam ≤ 1.5× cell_size). WANAKA mesh revalidation in `wanaka_step5_mc_revalidation.rs`. |
