...
**Last touched:** 2026-05-19 — Step 2 (D + H) landed: `Engagement` vector on every `SimulationCutSample`, per-kinematics summary block on per-toolpath and overall summaries, MCP per-span + per-depth-pass JSON surfaces gain inline `per_kinematics`. Legacy `radial_engagement` scalar carried as a derived view for one release; deletion follow-up scheduled.
