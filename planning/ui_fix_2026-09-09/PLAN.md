...
| F2.10 | **Opened by F2.4, 2026-09-10.** The SIMULATION arm of the same late-arrival race (R0.1 §5's `…_g_latesim`): a simulation result landing for a project edited since the run was submitted. Same shape as F2.4, different counter — `SimulationState::is_stale` against `GuiState::edit_counter`, not the per-toolpath `toolpath_revision`. Not covered by F2.4's row. | A sentry in the shape of the F2.4 arm: edit during a simulation, assert the arriving run is not presented as current. |
| F2.11 | **Opened by F2.5, 2026-09-10. Gate-trust item, not a UI defect.** `compute::worker::tests::cancelled_toolpath_returns_partial_debug_trace` (a cancellation-timing test in `rs_cam_viz`) failed once on a loaded machine during F2.5's gate run, then passed on four consecutive re-runs, and passed five more times when the orchestrator probed it in isolation. Nothing in F2.5 touches that module. A timing-sensitive test that fails under load is a threat to every gate result in this programme, because the whole verification method assumes a red result means a real regression. | Establish whether the test has a genuine race or merely a tight timeout under contention, and make it deterministic. Until then, a lone failure in this test on a loaded machine must be re-run before it is believed — and that re-run must be recorded, never silently taken as the answer. |
...
| F1.24 | F2.6 report | **Data-safety, core, needs an owner.** `ProjectSession::save` (`session/save.rs:59`) builds its atomic-write temp path as `.rs_cam_save_{pid}.tmp` in the PROJECT DIRECTORY, keyed on the process id alone. Two saves from ONE process into ONE directory therefore collide on the same temp file and one loses the rename. Found by the F2.6 worker, which it cost three failing tests before it was spotted; the tests now use separate directories, which is a workaround, not a fix. A per-save unique suffix is the obvious repair. Not this programme's — core file, outside the UI surface. |
| F2.12 | F2.10 report | **Safety surface, freshness gap.** The collision check carries NO staleness counter at all, so a holder-clearance verdict survives any edit still reading `Clear` — and it feeds `readiness::holder_clearance_check`, a SAFETY row. After F2.1–F2.10 the toolpaths, the export, the simulation and undo are all honest about being out of date; this verdict is not. Small, and squarely inside the model already built. |
| F4.4 | F4.3 report | **Wrong-cut path, pre-existing, found in passing.** `reload_model` drops `drill_targets` and `layers`, so reloading a drawing whose holes moved keeps the PREVIOUS version's drill targets — the program drills holes that are not in the file. Not a display defect. Core import surface. |
| F4.5, F4.6 | F4.3 report | Two smaller relink follow-ons; see `reports/F4.3.md`. |
...
### Opened 2026-09-10 by the three-lane build (J7/J8 + F2.12 + F4.4)

Each was found while doing another task. None was absorbed into that task.

| ID | Source | What |
|---|---|---|
| F4.7 | F4.4 lane | `rescale_model` invalidates NOTHING. A unit rescale moves every polygon and leaves every dependent toolpath result cached and green. R0.7 §3.3 asks the shared helper to invalidate; the F4.4 lane declined deliberately, because it is a different defect and a behaviour change. Priority raised by the F4.4 measurement: the door with the worst target defect is also the door that still ships stale results. |
| F4.8 | F4.4 lane | **Wrong-cut path, and F4.4 does NOT close it.** `DrillConfig::selected_holes` (`compute/execute.rs:1007-1012`) stores picked holes as frozen COORDINATES on the toolpath config, not as a reference into the model's targets. An operator who PICKED holes and then moved them in the drawing still drills the old positions after a reload, even with G-RELOADTARGETS landed. F4.4 repairs the default all-targets path only. |
| F2.13 | F2.12 lane | **A safety row answers for one toolpath and is labelled for the job.** `request_collision_check` (`controller/events/simulation.rs:375-395`) checks only the FIRST toolpath that has a result — one toolpath, one tool, one mesh — while the Readiness row and the pre-flight card both read "Holder clearance" for the whole program. A second tool with a longer holder is never asked about. Needs a compute change and a run, so the F2.12 lane recorded it rather than attempt it. |
| J8.1 | J7/J8 lane | **MCP still drops a pinned Top Z.** `session/compute.rs:4269` builds the diagnostics snapshot through `ResolvedHeights::from_context`, which projects the heights and loses a pinned Top Z. The generators cut `top_z - depth`, so on a pinned config the MCP route can miss a pocket cutting through the board. The GUI half now hands core the entry's RESOLVED heights and does not have this gap; the two surfaces therefore disagree on a pinned config. Core lane, changes the wire. |
| J8.2 | J8 core report | The diagnostics snapshot carries no `bottom_pinned` flag, so the three pin-honouring operations (Adaptive3d, UnifiedFinish, Waterline) ABSTAIN from the depth-beyond-stock rule rather than being answered for. Abstention is correct and is not a defect; adding the flag is small and would close the last three of twenty-four. |
| J8.3 | J7/J8 lane | The height diagram still draws a BZ line at a pinned bottom on operations that ignore the pin. The label column is 42 px at 9 pt and a longer label would overflow, so the J7 lane annotated the FIELD and left the DIAGRAM. Needs a rendering look, which needs the GUI. |
| J8.4 | J7/J8 lane | `gui_and_mcp_diagnostic_ids_match` is now a hand-rolled mirror of a list it no longer shares a source with. It references no deleted symbol, so it compiles and passes, but it has become the kind of parallel copy this programme exists to remove. |

### Opened 2026-09-10 by the second three-lane build (F1.24 + F2.13 + F4.8/F4.7)

| ID | Source | What |
|---|---|---|
| F2.14 | F2.13 lane | **Widen the holder-clearance check to every enabled operation.** F2.13 narrowed the CLAIM because the per-toolpath cost could not be established without a run. The cost of narrowing: a multi-operation job's holder row can no longer read `Pass`, and a mixed 2D/3D job cannot reach it at all. **A readiness dashboard that never goes green teaches an operator to ignore amber, so this is not cosmetic.** `run_collision_check` builds a `SpatialIndex` per call with no mesh dedupe and runs one `point_drop_cutter` per 1 mm sample per assembly segment for every cutting move, twice — measure that before widening. Operator decision, because it trades wall-clock for a green row. |
| F4.10 | F4.8 lane | The stale-pick refusal runs only when the model exposes at least one drill target, because an empty target list ALSO means "the caller resolved no model" at that seam (`execute.rs:3313` passes `&[]` on purpose, and all eight drill fixtures build models with empty targets). So the operator who DELETES every hole from the drawing still drills the picks. Pinned as a PASSING boundary test, so the gap is visible rather than assumed. |
