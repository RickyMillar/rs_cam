...
| **rest-grid anomaly** (P9 bullet 4) | Refining the rest cell makes the shipped detector find LESS feature and measured reach collapse toward zero. Pinned in `tests/rest_grid_resolution_c9.rs`. Owner: whoever next touches `rest_field::box_smooth_rest` / `nms_candidates` [owner re-pointed 2026-08-05, Checkpoint E ruling Q4/E10 — see `planning/review_2026-08-04/REST_GRID_ANOMALY_STUDY.md` §10; was `rest_field::measure_cross_section` — that function is downstream of the defect: its rim-quantisation contribution is bounded to one cell (≤0.5mm, ~0.25mm mean) and cannot by itself move reach by the logged 0.583mm, while `box_smooth_rest`'s fixed 3×3 window plus `nms_candidates`'s single-cell argmax displace the ridge O(h) inboard of the true rest maximum on an asymmetric peak; `rest_grid_resolution_c9` stays green and untouched until a replacement explains and supersedes it]. |
