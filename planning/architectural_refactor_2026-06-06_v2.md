...
| T11 | Phase 5 behavior adapters (one family at a time; drill-conversion dedup; cancellation regression test first) | 5 | IN PROGRESS (orchestrator, 2026-06-07) | Pre-cutover prerequisites DONE: cancellation net `cancellable_families_honour_a_preset_cancel_flag` (`a2b5df6` — all 4 polling families pinned to Err(Cancelled) on pre-set flag); fresh 54-sweep baseline captured (329 fingerprints, `target/param_sweeps/t11_baseline_pre.sha256`); AlignmentPinDrill cycle-conversion dedup (`59724ad`). Cutover infra landed (`c4e60fb`): `ExecutionContext` (14 bundled inputs) + `GenerateFn` + `OpRegistryEntry.generate: Option<GenerateFn>`; registry consulted first, exhaustive match kept as fallback + compile net with migrated arms delegating to the SAME fn (no divergence possible); migration state pinned by `generate_adapter_migration_is_an_explicit_per_op_decision` decision table. Migrated so far (fingerprints byte-identical to baseline per family): Drill + AlignmentPinDrill (`c4e60fb`), Waterline (`7110495` — first cancellable family, cancel net green through the registry path), Face (`078f5ff`), Pocket (`2c1d5de`), Profile (`5d70891`), Zigzag + Trace (`aec4939` — trace pins its family-specific annotate_trace_spans), VCarve + Chamfer (`93fb8cb` — V-bit InvalidTool refusals in-adapter), Rest + Inlay (`2020bc0`), DropCutter (`7227cd8` — cancellable). 13/23 done. Remaining 10: Adaptive, Adaptive3d, Pencil, Scallop, SteepShallow, RampFinish, SpiralFinish, RadialFinish, HorizontalFinish, ProjectCurve. Per-family recipe: move arm body verbatim into generate_<fam> adapter (ctx fields for prelude locals; KEEP the family annotate fn + span helper + refusals), wire registry literal via block-bounded Python replace, extend the decision-table test, gates = compute lib + family sweep fingerprint diff vs /tmp/t11_sweep_baseline_pre.sha256 (copy at target/param_sweeps/t11_baseline_pre.sha256) + clippy + fmt. Pencil@175° diffs are T14 noise — exclude from verdicts |
