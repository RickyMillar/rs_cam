...
| T14 | (discovered, T6 parity run) Pencil generator is nondeterministic at degenerate bitangency angles — same build, two runs: 7404 vs 7350 moves, 200 vs 177 z-levels at `bitangency_angle=175.0`. Undermines the fingerprint oracle for pencil; suspect rayon order-dependent accumulation or hash iteration in bitangency detection | follow-up | **DONE (`abb1696`, 2026-06-07)** — reproduced (same build, 2 runs: exactly the 3 known files diverged), root-caused to 3 HashMap-iteration-order leaks (NOT rayon): `compute_shared_edges` walking `edge_map` in RandomState order (propagates into chain-walker adjacency + walk priority), `chain_concave_edges` start-vertex order, and unvisited-loop seed order. Fix = ordering only (EdgeKey gains Ord; sort shared edges / start_vertices / unvisited_starts; closed-loop seed via `keys().min()`). Post-fix: 3 consecutive sweep runs byte-identical; ONLY the 3 unstable files changed vs pre-fix — every stable pencil fingerprint still matches the T11 baseline. Baseline copy's 3 pencil@175 lines refreshed (`target/param_sweeps/t11_baseline_pre.sha256`). Pencil@175° is no longer an excluded-noise carve-out in parity verdicts. Gates: pencil 10 + core lib 1783, clippy, fmt | reproduce with two same-build `sweep_pencil --ignored` runs; fix = deterministic ordering; gate = repeated runs byte-identical |
...
| T10 | Phase 4 `CutterOpProfile` aggregation + two-caller `SuggestContext` dedup | 4 | **DONE (`6d8b54f`+`bcdca97`+`65758b2`, 2026-06-07)** | PR 1: `feeds/profile.rs` — `CutterOpProfile::for_combo` aggregates shipped surface (`feeds_explain_for_operation` + `suggest_for_operation` with caller's exact context, 3 predict.rs predictors → `Predictions`, Phase-0 axial → `ConstraintEnvelopes`; feasibility = `FeedsError`, `RefuseReason` NOT shadowed); env-construction half of Suggest pass 0 extracted as `axial_envelope_for_operation` (per-op WOC/target/limit routing in ONE place, `pick_axial_envelope` consumes it — behavior-identical, pass ORDER in `enforce_invariants` untouched). PR 2: `ProjectSession::cutter_op_profile` owns the GUI/MCP context assembly; new wanaka sentry `session_cutter_op_profile_matches_gui_rationale_assembly` (per-toolpath byte-identical warnings + suggested op vs hand-rolled assembly). PR 3: feeds_modal `compute_suggest_rationale` + MCP `mcp_get_suggest_rationale` routed through it (per-caller lookup/error surfaces preserved; `compute_explain` untouched — no SuggestContext, 2nd caller in rollup). Excluded per plan: diagnostics `from_feeds`, `doc_derating_scale`, predict.rs calibrated constants (diff-verified 0 lines). Gates: wanaka 3/3, core 72 binaries 0 failing (incl. doctests), viz 189, cli, mcp, workspace clippy `-D warnings`, fmt |
| T16 | (discovered, T10 dedup) THIRD near-identical SuggestContext assembly in `cli/project.rs::apply_suggested_feeds_to_session` (`--apply-suggest`) — same {model_bbox, stock, ..default} shape but uses `SpindleStrategy::default()` where GUI/MCP use `session.post_config().spindle_strategy`. On projects with a non-default spindle strategy, CLI `--apply-suggest` and the GUI Suggest button produce different RPM/feed. Decide: bug (route through `cutter_op_profile`/post-config) or deliberate CLI semantics; `wanaka_suggest_integration.rs::run_suggest_for_enabled` mirrors the CLI variant today | follow-up | **DONE (`a383127`, 2026-06-07)** — decided BUG: the CLI's own `--spindle-strategy` flag writes `post_mut().spindle_strategy`, which the hardcoded `SpindleStrategy::default()` then ignored — the flag was a silent no-op for `--apply-suggest`, and projects with non-default strategy diverged from the GUI. Fix: `apply_suggested_feeds_to_session` restructured to two passes (immutable collect via `session.cutter_op_profile(tc)`, mutable apply + table); third hand-rolled SuggestContext assembly deleted; error surfaces + stderr table preserved. `wanaka_suggest_integration::run_suggest_for_enabled` rerouted through the same method — wanaka defaults to MatchChart so the value baseline is unchanged (structural re-baseline only). Live-verified: wanaka `--apply-suggest` matches sentry baseline; `--spindle-strategy max_speed` now shifts RPM/feed (16000→20000 rpm, 6912→8640 mm/min) where pre-fix it changed nothing. Gates: wanaka sentry 3, cli 5+9, workspace clippy, fmt | decision + (if bug) route through session method, re-baseline wanaka helper |
| T11 | Phase 5 behavior adapters (one family at a time; drill-conversion dedup; cancellation regression test first) | 5 | **DONE (`a2b5df6`..`845b985`, 2026-06-07)** | Pre-cutover: cancellation net `cancellable_families_honour_a_preset_cancel_flag` (`a2b5df6` — 4 polling families pinned to Err(Cancelled)); fresh 54-sweep baseline (329 fingerprints, copy at `target/param_sweeps/t11_baseline_pre.sha256`); AlignmentPinDrill cycle-conversion dedup (`59724ad`). Infra (`c4e60fb`): `ExecutionContext` (14 inputs) + `GenerateFn` + `OpRegistryEntry.generate`; registry consulted first, exhaustive match retained as compile net with arms delegating to the SAME adapter fns (divergence impossible). **All 23 families migrated** one-at-a-time, each proven against per-family sweep fingerprints: Drill+PinDrill `c4e60fb`, Waterline `7110495`, Face `078f5ff`, Pocket `2c1d5de`, Profile `5d70891`, Zigzag+Trace `aec4939`, VCarve+Chamfer `93fb8cb`, Rest+Inlay `2020bc0`, DropCutter `7227cd8`, Spiral/Radial/Horizontal `acface1`, Pencil/Scallop/SteepShallow/RampFinish `6d36288`, ProjectCurve `0ac3616` (setup_z_flipped caller-side pre-dispatch preserved), Adaptive+Adaptive3d `36ca171` (+`845b985` dead-prelude clippy fix — 36ca171's clippy gate was briefly red, restored next commit). Decision-table test now REQUIRES `Some` for all 23 (a new op must ship an adapter). FINAL ORACLE: full 54/54 sweep run, 326/329 fingerprints byte-identical to pre-cutover baseline; only diffs = the 3 known T14 pencil@175° files (same signature as T6 parity). Full core suite 0 failing binaries, viz 189/cli/mcp green, workspace clippy `-D warnings`, fmt. Family annotate fns + span helpers + refusals preserved per-adapter (trace/spiral/pencil/scallop/ramp/adaptive3d each pin their distinct span sources) |
| T12 | Phase 6 (6A): `MetricContext` into gates, collapse two assembly sites, `criteria()` slice, gating derives from criteria() | 6 | **DONE (`7c4407e`, 2026-06-07)** — 4 PRs: `beaecb2` tasks 4+5 (`criteria()` → `Vec<CriterionStatus>`; `CriterionStatus.exceeded: Option<ExceededCriterion>` set per gate; `exceeded_criteria`/`modeled_count`/`any_exceeded`/`any_unmodeled`/`all_not_applicable`/summary `exceeds_breakdown` ALL derive from `criteria()` — incl. `enforce_load_policy` via report fold; new sentries `gating_tier_derives_from_criteria` + `side_label_maps_chipload_sides_only`); `c029abc` task 2 (single verdict assembly site: `evaluate_toolpath` populates `modulation_summary` from trace — the silent divergence — and `gcode::project_load_report` builds `ToolpathLoadContext` + delegates; the 3 `rewrite_sim_required_to_stale_*` helpers still post-process); `0b8d478` tasks 1+3 (gates take `(ctx: &ToolpathLoadContext, env: &GateEnv{sim_trace, machine, tolerance})`, 7–10 positional args gone; machine-missing refusal moved INTO `power::evaluate` checked first for parity; pub `MetricEvaluator` trait + Chipload/Power/DeflectionGate handles; sentry `metric_evaluators_share_refusal_semantics`; in-module tests via per-module `evaluate_args` adapters, 48 sites); `7c4407e` task 5 completion (last 3 hand-folded `chipload||power||deflection` sites → `any_exceeded()`: optimize delta `candidate_is_safe`, viz Apply-enable, viz badge). Gates: full core suite 0 failing binaries (lib 1783 incl. tool_load 318 + diagnostics source-scan; integration sentries calibration/F-035/sim-invariant/F-040/litmatrix/F-036b green), viz 189, cli 5+9, mcp 2+5, workspace clippy `-D warnings` exit-0 verified per PR, fmt. MCP serde wire untouched — `ToolpathLoadVerdict` fields unchanged, `CriterionStatus` not serialized, so the anticipated snapshot churn did not materialize. | tool_load sentries + MCP serde tests |
