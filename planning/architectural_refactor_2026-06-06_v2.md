...
| T14 | (discovered, T6 parity run) Pencil generator is nondeterministic at degenerate bitangency angles — same build, two runs: 7404 vs 7350 moves, 200 vs 177 z-levels at `bitangency_angle=175.0`. Undermines the fingerprint oracle for pencil; suspect rayon order-dependent accumulation or hash iteration in bitangency detection | follow-up | IN PROGRESS (2026-06-07) | reproduce with two same-build `sweep_pencil --ignored` runs; fix = deterministic ordering; gate = repeated runs byte-identical |
