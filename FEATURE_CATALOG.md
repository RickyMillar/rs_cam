...
- **every feeds cell declares its basis** (feeds matrix, 2026-09-23) — `FeedsResult::support` reads `VendorBacked` when a printed vendor row answered, `FormulaOnly { source }` when the calculator's formula answered (the source string names the fit and the uncited k0), or `Refuse`. Suggest refuses the tool the operation's own registry row refuses (`tool_constraints`), the same rule the generator applies, and the refusal names the operation and the allowed kinds in words: non-V tools on VCarve, Inlay and Chamfer; a bull nose on Scallop and UnifiedFinish; a flat end mill, bull nose or V-bit on Pencil and SpiralFinish. The vendor table holds the printed chart rows with stored chart text and hashes; rows that no chart prints are labelled `derived`, grade C. Where a chart prints one value per size (Amana Spektra spiral plunge: flat-end pocket and adaptive at 1/8 in, 6 mm, 1/4 in) the row carries a maximum only, so Suggest shows no band and the post-simulation burn gate is silent on those cells. One published depth de-rate (Onsrud, Freud, Amana: 1.00 / 0.75 / 0.50 at 1 / 2 / 3 x D, linear between, 0.50 held above with a Caution) scales the feed, the shown band and the gate alike; the unsourced 1.5 x D depth hint is gone.
