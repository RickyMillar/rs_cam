...
impl ToolModelGpuData {
    ...
    pub fn from_tool_geometry(
        ...
        match geom.shape {
            ToolShape::FlatEnd => {
                ctx.draw_flat_end(&mut verts, tip_z, r, geom.cutting_length);
            }
            ToolShape::BallNose => {
                ctx.draw_ball_nose(&mut verts, tip_z, r, geom.cutting_length);
            }
            ToolShape::BullNose => {
                let cr = geom.corner_radius.min(r);
                ctx.draw_bull_nose(&mut verts, tip_z, r, cr, geom.cutting_length);
            }
            ToolShape::VBit => {
                ctx.draw_vbit(
                    &mut verts,
