...
impl EmbeddedCamServer {
    ...
    #[tool(
        ...
        description = "Strategy advisor: for the Adaptive3d roughing toolpath at `index`, compare clearing strategies (ContourParallel vs ContourSpiral) by planning each at its load-limited (Suggest-backed-off) params and timing it through the acceleration-aware cycle-time integrator at the machine's effective kinematics. Returns the recommended strategy, the binding regime (tool-/machine-limited) as the human-readable why, every candidate ranked by wall-clock seconds, and the speed margin. KEY: it decides parallel-vs-spiral by MACHINE ACCELERATION, not the load regime — on a low-accel router a backed-off ContourParallel beats the spiral even when tool-limited. Only applies to Adaptive3d ops. HEAVY: it plans one toolpath per candidate, so expect ~30-60 s. It runs on its own compute lane and holds no session, so the GUI stays usable while it computes; this call still waits for the answer."
    )]
    ...
    #[tool(
        ...
        description = "Run combined-Suggest against the toolpath at `index` and return the structured rationale tree explaining every parameter the orchestrator backed off or rewrote: DPP deflection back-off, runtime stepover floor, plunge-entry warnings, chipload-target feed lift, etc. Each entry carries param + reason + from/to values + a human-readable headline. `basis` gives the vendor support of the recipe as {headline, detail}: vendor-backed, formula only, refused, or an extrapolated G1 size claim whose detail states the scale, the rule, the valid range and the spread; `basis.hardness` is {headline, detail} when the hardness transfer is capped at the printed soft/hard ratio (G2), else null. `basis.plunge` is {source: {headline, detail}, value_mm_min, re_derived}: the plunge claim or named repo rule (G10), the plunge that ships, and {from_mm_min, to_mm_min, binding} when the funnel re-ran the plunge rule at the shipped feed, else null. `basis.entry` is one {headline, detail, caution} per entry rule (ramp angle, helix radius and pitch, the core or centre pip, the clearance, the straight-plunge caution), in card order. `basis.ramp` is {source: {headline, detail}, headline, detail, arm, value_mm_min}; `arm` is \"Sourced/<binding>\" (the G6 chip claim), \"PlungeSlope/<binding>\" (the G10 plunge slope; binding CutFeed, ChipTerm or PlungeTerm) or \"NoEntryFeed\", and null when no record was filed. Read this before set_toolpath_param when you want to know why Suggest chose a value. Does not mutate the project."
    )]
    ...
    #[tool(
        name = "inspect_spans",
        description = "Inspect the structural spans (Operation, DepthPass, Region, Entry, LeadOut, LinkBridge, DressupArtifact, GeometryRefit, RapidOrderBarrier) of a generated toolpath. With no filter, returns a summary: total `kind_counts` plus outermost spans (Operation + DepthPass) under `top_level` with child counts. Pass `kind`, `parent_id`, `pass_index`, or `region_id` to retrieve detail spans under `spans`. `kind` is a SpanKind name in snake_case. `parent_id` is a span id from a previous call (drill: Operation → DepthPass → Region). `pass_index` matches DepthPass payload; `region_id` matches Region payload. `max_spans` caps the result. `area_regions` lists the regions that a 3D Rough with By Area ordering detected (order = the Region span's region_id, cell_count, bbox_xy, surface and level Z ranges); it is null for every other toolpath and for Global ordering. Both arrays report their own bound in one vocabulary: the summary's `top_level_total_matching` / `_returned` / `_truncated` / `_cap`, and the detail's `spans_total_matching` / `_returned` / `_truncated` / `_cap`. A shortened array always says so. Run generate_toolpath first."
    )]
    ...
    #[tool(
        name = "add_tool",
        description = "Add a new tool to the project. Types: end_mill, ball_nose, bull_nose, v_bit, tapered_ball_nose. The geometry that DEFINES the tool for its type is REQUIRED, not defaulted — `included_angle` for v_bit (a 20-degree V-bit is 20.0), `taper_half_angle` for tapered_ball_nose (cone HALF angle), `corner_radius` for bull_nose — and the call is refused if it is missing, because guessing one silently creates a different cutter. Optional: flute_count (2), cutting_length (25 mm), shaft_diameter (6.35 — the cutting ENVELOPE of a tapered ball), shank_diameter (6.35), shank_length (20), stickout (45), holder_diameter (25), tool_number, size_units ('metric' or 'imperial': the unit the size is SHOWN in; omitted, it is inferred from the name, and every length stays mm). Every value NOT supplied is listed in the reply's `defaulted` array — read it, those defaults are generic, not your tool. `tool_number` auto-allocates to the next free number in the project so M6 tool changes re-trigger; geometry belonging to another tool type is stored as zero rather than a misleading placeholder. Returns the new tool index."
    )]
    ...
    #[tool(
        ...
        description = "Set dressup configuration for a toolpath. Pass a JSON object with dressup fields: entry_style, ramp_angle, helix_radius, helix_pitch, entry_clearance_mm, dogbone, dogbone_angle, lead_in_out, lead_radius, lead_in_feed_rate, lead_out_feed_rate, link_moves, link_max_distance, link_feed_rate, arc_fitting, arc_tolerance, segment_merge, segment_merge_tolerance, feed_optimization, feed_max_rate, feed_ramp_rate, optimize_rapid_order, air_bridge_policy. helix_radius is in mm; null = 0.3 x tool diameter (repo rule), and the engine caps every helix radius at the tool's flat bottom so the helix leaves no core."
    )]
    ...
    #[tool(
        name = "reach_map",
        description = "Does this tool's tip fit into the valleys? For one FINISHING toolpath (a mesh operation that is not a roughing pass), drop its cutter over the model surface and report which of that surface the tool can form inside the operation's tolerance and which it cannot. Replies with the area-weighted unreachable percentage, the worst gap in mm, the measured area against the model's total area, a gap histogram, and the grid the answer was taken on. Reads only — nothing is generated and no parameter moves. Three readings an agent must not get wrong. (1) `measured_area_mm2` well below `surface_area_mm2` is EXPECTED, not a fault: this is a top-down measure, so undersides, walls and a band one envelope radius wide inside the part outline are NOT MEASURED, never `reachable`; check `is_measured` before believing a clean percentage. (2) READ `grid_note` FIRST — it is the first line of the reply and it names the cell, the floor and the bar. `discretisation_floor_mm` is what this grid can resolve ON THIS SURFACE: the larger of the plane-only `profile_floor_mm` and the curvature term `curvature_floor_p95_mm`, which is the one that bites (order `cell^2 / (8 (rho - R))` on a concave feature of radius rho — 0.05 mm at a 0.645 mm cell, the whole default tolerance). When `tolerance_below_floor` is true the bar is UNDER the arithmetic. The grid's gap bias is NON-NEGATIVE — a minimum over a sampled CL set sits at or above the continuum minimum — so `unreachable_pct_of_measured_area` OVER-states: the true unreachable share is AT OR BELOW it, `unresolved_pct_of_measured_area` is the band the grid cannot classify either way, and `reached` is the sound side (a cell called reached really is formed). Raise the tolerance rather than reading the residual as tool geometry. Every percentage is 3D-SURFACE-AREA weighted over the rim-eroded population (`area_basis`); a planar or whole-board figure beside one of these is a different question and on a terrain that alone is 5 points. The CELL follows the tool's tip sphere and the model bbox and NEVER the tolerance, so a series of probes at moving tolerances is comparable; percentages from two different tools or models are not. (3) `tolerance_mm` defaults to the operation's own declared cusp / scallop height, else — for a raster finish on a spherical tip — the CUSP ITS OWN STEPOVER LEAVES, `R - sqrt(R^2 - (s/2)^2)` on the TIP radius, else 0.05 mm. Never `stock_to_leave`, which is an intended offset rather than a bar. `tolerance_source` and `tolerance_note` say which rung applied; pass `tolerance_mm` to probe another. Cost: a cold map is a full-grid drop-cutter walk, roughly a second or two on a 200 mm board, and it shares its memo with the viewport overlay and with screenshot_toolpath's `reach_overlay`, so the second call on the same key is free."
    )]
    async fn reach_map(
        &self,
    ...
    #[tool(
        ...
        description = "Set the live machine profile's kinematics directly, without a GRBL dump: per-axis acceleration (acceleration_x/y/z_mm_s2, GRBL $120/$121/$122), per-axis maximum rate (max_rate_x/y/z_mm_min, GRBL $110/$111/$112), junction_deviation_mm ($11), and optionally the isotropic fallback acceleration_mm_s2, max_junction_velocity_mm_min and jerk_mm_s3. Every field is optional and omitted fields keep their current value; when all three axis accelerations are given, the isotropic scalar follows their mean unless you pass one. Pass the acceleration triple and the rate triple whole, or patch one axis of a triple the machine already carries. Neither is cosmetic — acceleration drives the cycle-time integrator and the parallel-vs-spiral verdict from `recommend_clearing_strategy`, and the max rates cap every move's cruise velocity, so a Z-dominant move runs at $112 (typically 1000 mm/min) instead of the X/Y travel rate. Breaks any machine-library link (the values become inline). For a raw `$$` dump use `import_machine_settings` instead. Verify with inspect_machine."
    )]
