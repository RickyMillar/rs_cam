...
impl<B: ComputeBackend> AppController<B> {
    ...
    /// Step the playhead by ±1 [`SpanKind::DepthPass`] on the focused
    /// toolpath. Anchors at the span's `start_move`. No-op when the focused
    /// toolpath has no DepthPass spans (e.g. a single-pass operation, or a
    /// toolpath whose generator does not emit pass spans).
    pub(crate) fn handle_sim_jump_pass(&mut self, delta: i32) {
        use rs_cam_core::toolpath_spans::{Span, SpanKind};
        if !self.state.simulation.has_results() {
            return;
        }
        let Some(focused) = self.state.simulation.focused_toolpath() else {
            return;
        };
        let current_global = self.state.simulation.playback.current_move;
        let (current_local, _) = match self
            .state
            .simulation
            .move_to_local_toolpath_move(current_global)
        {
            Some((_, id, local)) if id == focused => (local, id),
            _ => (0usize, focused),
        };
        let Some(rt) = self.state.gui.toolpath_rt.get(&focused.0) else {
            return;
        };
        ...
        let mut passes: Vec<&Span> = result
            .spans()
            .iter()
            .filter(|s| matches!(s.kind, SpanKind::DepthPass))
            .collect();
        passes.sort_by_key(|s| s.start_move);
        if passes.is_empty() {
            return;
        }

        let cur_idx = passes.iter().rposition(|s| s.start_move <= current_local);
        let target_idx = match cur_idx {
            Some(i) => {
                if delta > 0 {
                    (i + 1).min(passes.len().saturating_sub(1))
                } else {
                    i.saturating_sub(1)
                }
            }
        ...
        let Some(span) = passes.get(target_idx) else {
            return;
        };
        let target_local = span.start_move;
        let Some(target_global) = self
            .state
            .simulation
            .global_move_for_local(focused, target_local)
        else {
            return;
        };
        self.state.simulation.playback.playing = false;
        self.state.simulation.playback.current_move = target_global;
    }
