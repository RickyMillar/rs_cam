...
// ── The rest-stock chain fixture ─────────────────────────────────────────

/// One simulation request, as the fake records it: per group, how many
/// toolpaths the include filter admitted and what the phantom scan picked.
type SimRequestLog = Vec<(usize, Option<(usize, ToolpathId)>)>;

/// A backend that models the one rule that makes a plan necessary: prior
/// stock for an operation appears only when a **simulation** runs AFTER its
/// predecessor has generated. Toolpath submits succeed immediately.
///
/// W1 un-gated it from `mcp`: the GUI sink has no oneshot, so the plan sentry
/// drives it without the feature.
struct RestChainBackend {
    drained: Vec<ComputeMessage>,
    pub simulations: usize,
    /// Per simulation, per group: how many toolpaths the include filter
    ...
    fn submit_simulation(&mut self, request: SimulationRequest) {
        ...
        self.sim_requests.push(
            request
                ...
                .groups
                .iter()
                .map(|group| (group.toolpaths.len(), group.phantom_prior_stock))
                .collect(),
        );
        ...
        // The real simulator publishes a snapshot for every operation it
        // carved, plus the one the phantom scan picked. W1: read the
        // REQUEST's own scan rather than re-deriving a chain rule here, so
        // the sentry proves "one unlock per setup per simulation" against
        // the rule the production builder obeys.
        let mut prior_stocks = std::collections::HashMap::new();
        for group in &request.core.groups {
            for entry in &group.toolpaths {
                prior_stocks.insert(entry.id, stock());
            }
            if let Some((_, phantom)) = group.phantom_prior_stock {
                prior_stocks.insert(phantom, stock());
            }
        }
        self.drained
            .push(ComputeMessage::Simulation(Ok(Box::new(SimulationResult {
...
fn pump_until_resolved(
    ...
    panic!("generate_all never resolved — the plan is not terminating");
}

/// Drive the controller until no plan is in flight, or give up.
///
/// The GUI sink has no oneshot, so [`pump_until_resolved`]'s receiver is
/// unavailable. Same iteration cap, same failure.
fn pump_until_idle(controller: &mut AppController<RestChainBackend>) {
    for _ in 0..200 {
    ...
    panic!("the generation plan never finished — the driver is not terminating");
}
