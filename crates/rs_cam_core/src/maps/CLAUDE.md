...
- The reach map is a top-down, rim-eroded UPPER estimate. Read the grid note
  and the measured area first. Red at or below the discretisation floor is
  unresolved arithmetic, not proven geometry.
...
- `cargo test -p rs_cam_core --features test-support -q --test reach_map_p5`
  (also the FLD-01 wire shape), and the same line for
  `reach_map_residual_p5_1`, `tier_map_walk_t1` and `tier_map_slope_t2`.
