...
- `vendor_lut.rs`, `vendor_lookup.rs`, `vendor_normalize.rs` — vendor table, lookup, type mapping. `ramp.rs` — G6 ramp: `RampBasis`, the entry θ, `resolve_ramp_feed`, the G10 `entry_notes`. `plunge.rs` — G10: `PlungeBasis`, `resolve_plunge`.
- `extrapolation.rs` + `extrapolation/{size,hardness,family,drill,plunge}.rs` — `SizeLaw` (G1 size claim, form A/B/C or refusal),
  `hardness_basis` (G2 Janka + soft/hard cap), `family_basis` (G3 `FAMILY_RULES`), `drill_basis` (G6 `DRILL_RULES`) run in `build_result`; `plunge_rule` (G10 `PLUNGE_RULES`).
- `profile.rs`, `quantities.rs`, `provenance.rs`, `support.rs` — the pre-sim profile, the boundary newtypes, the per-value provenance, the `FeedsSupport` arm. `rationale.rs`, `feed_explanation.rs`, `explain_payload.rs` — rationale tree, stage record, UI contract.
...
- G6 (B5): no drill rows, no drill multiplier. `DRILL_RULES` (Amana Spektra flat, 3.0-12.7 mm, 2/3 F, widened 2026-09-25) read the (Pocket, Roughing) side row x 1/Z (a point stays a point); the drill RPM cap replaces the chart's 18 000; every other drill cell refuses, in every material. G6 ramp: `FeedsResult::ramp` holds the axial chip; `suggest::apply` writes `ramp_feed_rate = floor(min(shipped feed, chip x shipped rpm x Z / tan θ))`; no clamp rule.
- G10: plunge = the family fraction x shipped feed inside `PLUNGE_RULES` (flat 1/Z on the G6 range, ball and tapered 0.50, 60° V-bit 1/3), else the named material base; ball and tapered tip cap 150/mm; the funnel re-runs it at the shipped feed. Non-G6 ramp = floor(min(F, plunge / tan θ)); a straight plunge or unknown entry writes `None`. Suggest never writes the entry style (Q11). Part B: θ and the helix notes read the emitted r (`helix_radius_for`: rule 0.3 x the nominal D, `helix_entry_diameter_mm` (a tapered ball's tip, not its shaft), capped at the flat bottom), so the card states the cap, not a core.
...
- R4: the rubbing floor `min(0.025, band min)` warns, never lifts; no machine or L/D feed factor. The dial `MachineProfile::aggressiveness` (0.85) scales depth and stepover to hold load at `k × L/D share`; it never cuts the feed. Power ceiling = `power_at_rpm`, no fraction.
- B6: force, power, efficiency and the deflection predictor read one line, `Material::force_line()`; power is `line × grain_factor` (1.0 on every line), deflection the line alone. No `Kc`, no anchor. A refusal is `MaterialUnvalidated`; a chip outside the printed range extends the line and `PowerFigure::force` names the `ChipRegime`.
...
`a_flat_end_plunge_is_the_side_chip_over_z_g6`, `a_ramp_feed_is_the_g6_chip_over_the_entry_slope_g6ramp`, `a_vbit_row_is_read_at_its_printed_key_b4`, `the_force_line_is_printed_per_family_b6`,
`a_plunge_is_a_named_fraction_of_the_side_feed_g10`, `a_ramp_off_the_g6_claim_holds_its_vertical_rate_at_the_plunge_g10`, `suggest_leaves_the_entry_style_to_the_operator_g10`, `a_helix_leaves_no_core_and_a_ramp_never_outruns_the_feed_g10`. `feeds_matrix_instrument_fm1` is an `#[ignore]` instrument (`-- --ignored`) that writes planning/feeds_matrix_2026-09-23/. `wanaka_suggest_integration` takes minutes: ask first.
