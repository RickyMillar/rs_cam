...
(`support::formula_backing` is the R1 judgement), `finish_depth_is_reported_not_capped_fm6` (R2: no finishing depth cap; `geometry::depth_cap_diameter_mm` is the one cap diameter). `feeds_matrix_instrument_fm1` is an `#[ignore]` instrument (`-- --ignored`) that writes planning/feeds_matrix_2026-09-23/. `wanaka_suggest_integration` takes minutes: ask first.
