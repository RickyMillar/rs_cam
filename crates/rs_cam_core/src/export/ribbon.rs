//! Toolpath ribbon geometry and the per-vertex colour ramps.
//!
//! STK-14: these builders sat in `stock/stock_mesh.rs`, where five of the
//! file's eleven public items described a picture rather than the stock. The
//! core contract keeps the crate GUI-free, and `export/` is the folder that
//! owns output that is not G-code — and was already the only core reader.
//!
//! [`StockMesh`] itself stays in `stock/`: it is a mesh container, and the
//! dexel mesh extractors fill it.
...
/// than wood tone when the question is depth variation.
pub fn apply_height_gradient(mesh: &mut StockMesh) {
    let colors = height_gradient_colors(&mesh.vertices);
    mesh.colors.clear();
    mesh.colors.reserve(colors.len() * 3);
    for [r, g, b] in colors {
        mesh.colors.push(r);
        mesh.colors.push(g);
        mesh.colors.push(b);
...
/// Copy `mesh` with every vertex colour scaled by `factor`, to dim a
/// background behind a ribbon.
pub fn dimmed(mesh: &StockMesh, factor: f32) -> StockMesh {
    StockMesh {
