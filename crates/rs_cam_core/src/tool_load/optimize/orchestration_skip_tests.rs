...
fn custom_material_yields_skipped() {
    ...
    // the feed-scale scalar — any Custom value triggers the
    // MaterialUnvalidated skip. A custom material also has no force
    // line (ruling B6 removed its typed `kc` field).
