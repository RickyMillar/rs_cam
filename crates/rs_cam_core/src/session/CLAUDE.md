...
- `mutation.rs` and `mutation/` — the CRUD methods, in three children.
- `compute.rs` and `compute/` — generation, simulation, export, diagnostics.
- `diagnostics_types.rs` — the diagnostic types. Keep the `mod.rs` re-export.
- `builder.rs`, `project_file.rs`, `save.rs` — construction, the file.
- `eval_context.rs`, `cycle_time.rs`, `reach.rs`, `multitool.rs` — context,
  cycle time, reach, the planner.
