...
/// Plan §3.1 and §3.7: the adapter is the one gate of the step ladder. It
/// refuses a list that is not strictly descending, a coarse step that is
/// not above Depth/Pass, and a ladder on a strategy other than contour
/// parallel. An empty ladder passes on every strategy.
#[test]
fn the_adaptive3d_adapter_refuses_a_bad_step_ladder() {
    use super::finish_3d::check_adaptive3d_step_ladder;
    use crate::compute::operation_configs::{Adaptive3dConfig, ClearingStrategy};

    let refusal = |cfg: &Adaptive3dConfig| match check_adaptive3d_step_ladder(cfg) {
        Err(OperationError::Other(msg)) => msg,
        other => panic!(
    ...
    let ascending = Adaptive3dConfig {
        depth_per_pass: 1.0,
        coarse_steps: vec![5.0, 10.0],
        ..Adaptive3dConfig::default()
    };
    assert!(refusal(&ascending).contains("not strictly descending"));

    let not_above_base = Adaptive3dConfig {
        depth_per_pass: 5.0,
        coarse_steps: vec![3.0],
        ..Adaptive3dConfig::default()
    };
    assert!(refusal(&not_above_base).contains("larger than Depth/Pass"));

    let zero = Adaptive3dConfig {
        depth_per_pass: 1.0,
        coarse_steps: vec![0.0],
        ..Adaptive3dConfig::default()
    };
    assert!(refusal(&zero).contains("not a finite step above zero"));

    let wrong_strategy = Adaptive3dConfig {
        depth_per_pass: 5.0,
        coarse_steps: vec![10.0],
        clearing_strategy: ClearingStrategy::Adaptive,
        ..Adaptive3dConfig::default()
    };
    let msg = refusal(&wrong_strategy);
    assert!(
        msg.contains("contour parallel") && msg.contains("Adaptive"),
        "{msg}"
    );

    let empty_on_adaptive = Adaptive3dConfig {
        clearing_strategy: ClearingStrategy::Adaptive,
        ..Adaptive3dConfig::default()
    };
    assert!(check_adaptive3d_step_ladder(&empty_on_adaptive).is_ok());

    let good = Adaptive3dConfig {
        depth_per_pass: 1.0,
        coarse_steps: vec![10.0, 5.0],
        clearing_strategy: ClearingStrategy::ContourParallel,
        ..Adaptive3dConfig::default()
    };
    assert!(check_adaptive3d_step_ladder(&good).is_ok());
}
