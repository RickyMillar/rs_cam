...
use crate::ops::waterline::waterline_contours_with_cancel;
...
/// How one level treats a cell (step-ladder plan §3.2, fit rule v2 of
/// Phase 2c).
...
pub(super) enum LevelRule {
    ...
    #[default]
    Drape,
    /// A coarse tier: the fit rule v2, "pocket depth fits" (ruled
    /// 2026-09-24). The level cuts the slab `(slab_top_z, z_level)`. Let
    /// `floor = surf + stock_to_leave`:
    ///
    /// - `floor <= z_level`: the slab fits. The cell is eligible and the
    ///   level cuts it at `z_level`.
    /// - `z_level < floor < slab_top_z`: the floor is inside the slab. A
    ///   gentle cell (slope below [`FIT_WALL_SLOPE_DEG`]) is eligible, and
    ///   the drape takes the cut down to its floor in the same pass. A
    ///   steep cell is a wall: a keep-out, and the finer tiers step it.
    /// - `floor >= slab_top_z`: the part is above the slab. A keep-out.
    ///
    /// An eligible cell also needs stock above `max(floor, z_level)`.
    /// `surf` is the per-cell drop-cutter rest height of the operation's
    /// cutter, so the test is aware of the whole footprint: a tool centre
    /// on an eligible cell cannot touch the part below its floor.
    Clip {
        /// The top of the slab that the level cuts.
        slab_top_z: f64,
    },
}

/// The slope from which a cell whose floor is inside a coarse slab is a
/// wall (a keep-out), not a floor that the coarse pass cuts to (fit rule
/// v2). 30° is the start value of the ruling, the old Mill Shallow
/// default. A floor below it is flat enough that the drape of one coarse
/// pass follows it with a bite of at most one step. A steeper cell is the
/// wall band, which the finer tiers cut in terraces. Named, not a dial.
pub(super) const FIT_WALL_SLOPE_DEG: f64 = 30.0;
...
pub(super) struct MaterialBoolGrid {
    ...
    /// [`LevelRule::Clip`] only: `true` where the level may not cut (a
    /// wall inside the slab, or the part above the slab), in the same
    /// padded layout. The region filter does not apply to it: the wall is the
...
/// [`LevelRule::Clip`]: a cell is `true` only if the fit rule v2 admits
/// it; the other cells go into the keep-out mask. The grid is
...
fn build_material_bool_grid(
    ...
    let (mut keep_out, slab_top_z) = match rule {
        LevelRule::Drape => (None, f64::INFINITY),
        LevelRule::Clip { slab_top_z } => {
            (Some(vec![false; padded_rows * padded_cols]), slab_top_z)
        }
    };
    ...
    for row in 0..rows {
        for col in 0..cols {
            ...
            // The fit rule v2: on a clip level a cell whose floor is above
            // the slab, or a steep cell whose floor is inside it, is a
            // keep-out.
...
/// smaller than `min_extent_cells` on both axes. Returns the number of
...
pub(super) fn clear_z_level_contour_parallel(
    ...
    // A clip level also cuts gentle floors inside its slab (fit rule v2),
    // and this gate counts only the cells whose floor is below the level.
    // The count of the material grid below gates a clip level instead.
    let is_clip = matches!(ctx.level_rule, LevelRule::Clip { .. });
    if !is_clip && remaining.cells_with_material < MIN_CELLS_TO_CLEAR {
        debug!(
            z = z_level,
    ...
    if mat_count == 0 || (is_clip && (mat_count as u64) < MIN_CELLS_TO_CLEAR) {
    ...
    // A clip level cuts at `z_level`, or at the floor of a gentle cell
    // inside the slab (fit rule v2, A2): the blend toward the terrain
    // would take inner rings below the slab.
...
/// The regions one Z level clears, in the order the cutter visits them, and
/// the plan metrics the detection stage already knows.
///
/// CUT-09: the product of stage 1 of `clear_z_level_agent_2d_slice`.
struct OrderedRegions {
    regions: Vec<crate::polygon::Polygon2>,
    metrics: ZLevelPlanMetrics,
}

/// Stage 1 — find the regions this Z level must clear and order them.
///
/// The material boolean grid at `z_level`, its marching-squares contours,
/// the containment grouping into outer regions with their holes, the
/// sub-tool area filter and the nearest-neighbour visit order. Reads the
/// stock; changes nothing.
///
/// `None` means there is nothing left to cut at this level.
fn detect_and_order_regions(
    ctx: &ClearZLevelContext<'_>,
    material_stock: &TriDexelStock,
    surface_hm: &SurfaceHeightmap,
    z_level: f64,
    ...
    let level_metrics = ZLevelPlanMetrics {
        available: true,
...
/// The four accumulators a Z level appends to, plus the two counters and
/// the metrics record its stages share.
///
/// CUT-09: `clear_z_level_agent_2d_slice` used to carry these as four
...
struct RegionClearPlan<'a> {
    surface_hm: &'a SurfaceHeightmap,
    z_level: f64,
    params_2d: &'a crate::adaptive::AdaptiveParams,
    level_ctx: &'a Option<ToolpathDebugContext>,
    region_total: usize,
}

/// Stage 2 — clear ONE region: the cut-length forecast, the perimeter
/// sweep, the 2D adaptive walk and the residual cleanup, each lifted to 3D
/// and stamped into the planner's dexel state.
///
/// Each region's first 2D segment is a Rapid (the entry into that region),
/// which becomes a 3D Rapid: retract to safe_z, traverse XY, plunge. Between
/// disjoint regions that retract is correct; the 2D adaptive cannot link
/// across regions because each call sees only its own region's polygon.
// SAFETY: every `path_2d[i]` and `pts[i]` below indexes a vector this
// function just built, at an index its own `len()` guard admits.
#[allow(clippy::indexing_slicing)]
fn clear_one_region(
    ctx: &ClearZLevelContext<'_>,
    plan: &RegionClearPlan<'_>,
    emission: &mut LevelEmission<'_>,
    region_polygon: &crate::polygon::Polygon2,
    region_idx: usize,
    cancel: &dyn CancelCheck,
) -> Result<(), Cancelled> {
    ...
    let dropped_short_regions = &mut *emission.dropped_short_regions;
    let surface_hm = plan.surface_hm;
    let z_level = plan.z_level;
    let params_2d = plan.params_2d;
    let level_ctx = plan.level_ctx;
    let region_total = plan.region_total;
    ...
    let inset_polygons =
        crate::polygon::offset_polygon(region_polygon, ctx.tool_radius + PERIMETER_INSET_MARGIN_MM);
                ...
                if !engaged.is_empty() {
                    ...
                    while i < engaged.len() {
                        ...
                        if run_len_mm < min_air_run_mm {
                            for is_engaged in engaged.iter_mut().take(run_end + 1).skip(run_start) {
                                *is_engaged = true;
                            }
                        }
                        i = run_end + 1;
...
pub(super) fn clear_z_level_agent_2d_slice(
    ...
    let mut emission = LevelEmission {
        ...
        dropped_short_regions: &mut dropped_short_regions,
    };
    let plan = RegionClearPlan {
        surface_hm,
        z_level,
        params_2d: &params_2d,
        level_ctx: &level_ctx,
