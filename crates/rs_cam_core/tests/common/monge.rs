...
//!
//! **Extraction, not new math.** This module extracts the estimator that
//! `tests/wanaka_curvature_anisotropy.rs` (commit `7f341f9d`) built and
//! `tests/zone_coherence_census.rs` restated. Those two files carry the full
//! derivation in their module docs; this module carries the code so a third
//! census does not fork-and-drift the arithmetic. Per the migration policy
//! at the top of `common/mod.rs`, the two wanaka censuses keep their own
//! verbatim copies — they are evidence instruments and stay untouched.
//!
