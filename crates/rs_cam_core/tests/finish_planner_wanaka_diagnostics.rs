//! Wanaka decomposition DIAGNOSTICS — tables, not gates.
//!
//! FIN-05: these three tests lived beside the R1 acceptance gate in
//! `finish_planner_wanaka_decompose.rs`, which the folder `CLAUDE.md` named
//! as a sentry. Two of them say in their own headers that they assert
//! nothing, and the third asserts only determinism. A reader who ran the
//! "sentry" ran three research tables with it.
//!
//! They keep every line they had. What changed is where they live and when
//! they build: this target needs `heavy-tests`, so the dev loop neither
//! compiles nor runs it.
//!
//! Run one with:
//! `cargo test -p rs_cam_core --features heavy-tests --test finish_planner_wanaka_diagnostics -- --ignored --nocapture`
...
/// The pinned wanaka project these tables run on — the same fixture the R1
/// gate reads (FIN-06). `planning/` is evidence, not a fixture store.
