//! Avenue F, phase E2 — the E1 chain on TIER-0 MOUNTAIN GROUND
//! (`planning/metrology_2026-09-02/FINDINGS.md` §M6, pre-registered).
//!
//! The operator caught a territory error in the whole M2–M5 chain: every
//! instrument ran on the tier-1 fine island — the dendritic fringe the
//! valley harness selected — never on the mountains. This is the same
//! instrument as `graded_raster_e1.rs` retargeted at the TIER-0 island
//! with the R1.5 coarse tool: censuses first (band shares; flat-to-steep
//! decorrelation), then arm S vs the graded-raster W sweep under the
//! UNCHANGED §M5 bars, then SVG dumps for the operator's eyeball.
//!
//! The link ceiling is the ROUGH-ONLY stock: tier 0 cuts on roughed
//! ground (tier 1 cuts after tier 0, which is what E1's ceiling
//! modelled).
//!
...
fn machined_stock(
    ...
    for row in 0..rows {
        ...
        for col in 0..cols {
            ...
            let top = rough_z + ROUGH_STOCK_TO_LEAVE_AXIAL_MM;
            if top < block_top_z {
...
/// Ramer–Douglas–Peucker polyline simplification, 3D perpendicular
/// distance, tolerance in mm. The graded passes carry cell-level jitter
/// (dense 0.25 mm points) that pays junction slowdown the control's
/// collinear rows never pay; simplification to the OP TOLERANCE removes
/// the jitter and keeps the grading. Applied to EVERY arm identically.
fn simplify_run(run: &[P3], tol: f64) -> Vec<P3> {
    if run.len() < 3 {
        return run.to_vec();
    }
    ...
    while let Some((i0, i1)) = stack.pop() {
        ...
        let (mut worst, mut at) = (0.0_f64, i0 + 1);
        for i in i0 + 1..i1 {
            let d = seg_dist(run[i], run[i0], run[i1]);
            if d > worst {
                worst = d;
                at = i;
            }
...
fn wanaka_graded_raster_tier0_e2() {
    eprintln!(
        "\n========== Avenue F, E2 — GRADED RASTER ON TIER-0 MOUNTAIN GROUND ==========\n\
         Pre-registration: planning/metrology_2026-09-02/FINDINGS.md §M6\n\
         (bars unchanged from §M5). NUMBERS ONLY.\n"
    );
    ...
    .expect("islands");
    // Tier 0 is the COMPLEMENT: it has no TierIslandSet (the
    // coarse_skips_fine_islands arm) — its territory is the covered board
    // minus every fine tier's OWNED polygons.
    let fine_owned: Vec<&rs_cam_core::geometry::region_set::RegionSet<'static>> =
        islands.per_tier.iter().map(|set| &set.owned).collect();

    let surface = build_classification_surface_with_sampler_and_cancel(
        &mesh,
        &index,
        &r15,
        unified_finish_classification_resolution(&r15, OP_TOLERANCE_MM),
    ...
    let covered: Vec<bool> = heightmap
        ...
        .map(|(i, &flag)| {
            ...
            !fine_owned.iter().any(|owned| owned.contains(&p))
        })
        .collect();

    let mut planner = FinishPlannerParams::for_tool(cusp_radii[0]);
    ...
    let stepover = equal_cusp_stepover_mm(cusp_radii[0], CUSP_HEIGHT_MM);
    ...
    let regime = LinkRegime {
        ...
        ceiling: Some(LinkCeiling {
            stock: Some(&stock),
            tool_radius: r15.envelope_radius_mm(),
    ...
    // Band-share census: how much of tier-0 rasters (Shallow) vs already
    // cuts contour-like (MidSteep scallop / VerySteep waterline)?
    {
        let mut counts = [0usize; 4];
        for l in &planned.labels {
            match l {
                Some(FinishBand::Shallow) => counts[0] += 1,
                Some(FinishBand::MidSteep) => counts[1] += 1,
                Some(FinishBand::VerySteep) => counts[2] += 1,
                None => counts[3] += 1,
            }
        }
        let tot = (counts[0] + counts[1] + counts[2]).max(1);
        eprintln!(
            "  band shares (tier-0 cells): Shallow {:.1} %, MidSteep {:.1} %, \
             VerySteep {:.1} %  ({} covered cells, {:.0} mm² XY)",
            100.0 * counts[0] as f64 / tot as f64,
            100.0 * counts[1] as f64 / tot as f64,
            100.0 * counts[2] as f64 / tot as f64,
            tot,
            tot as f64 * cell_area
        );
    }
    // Contour-alignment census (the operator's topo intuition, measured):
    // how fast does the allowed-pitch field vary ALONG terrain contours vs
    // ACROSS them? Fronts aligned with the grain see the along rate; the
    // 0-degree graded rows of this instrument see a mix. If along <<
    // across, contour-aligned graded fronts (the river-offset family) get
    // the refund without the wander.
    {
        let a_of = |i: usize| stepover * theta_deg[i].to_radians().cos();
        let mut along: Vec<f64> = Vec::new();
        let mut across: Vec<f64> = Vec::new();
        for row in 1..rows - 1 {
            for col in 1..cols - 1 {
                let i = row * cols + col;
                if !covered[i] || region_of[i].is_none() {
                    continue;
                }
                let n = slope.normals[i];
                let g = (n.x).hypot(n.y);
                if g < 1e-6 {
                    continue; // flat: no defined grain
                }
                let (gx, gy) = (n.x / g, n.y / g); // downhill XY direction
                let ok = |j: usize| covered[j];
                let (ixp, ixm) = (i + 1, i - 1);
                let (iyp, iym) = (i + cols, i - cols);
                if !(ok(ixp) && ok(ixm) && ok(iyp) && ok(iym)) {
                    continue;
                }
                let dax = (a_of(ixp) - a_of(ixm)) / (2.0 * cell);
                let day = (a_of(iyp) - a_of(iym)) / (2.0 * cell);
                // gradient of a projected on grain (across contours) and on
                // the contour tangent (along).
                across.push((dax * gx + day * gy).abs());
                along.push((dax * -gy + day * gx).abs());
            }
        }
        let stat = |v: &mut Vec<f64>| -> (f64, f64) {
            ...
            (mean, v[((n - 1) as f64 * 0.9) as usize])
        };
        let (mean_along, p90_along) = stat(&mut along);
        let (mean_across, p90_across) = stat(&mut across);
        eprintln!(
            "  contour-alignment census ({} cells): |da/ds| ALONG contours mean \
             {mean_along:.4} p90 {p90_along:.4} mm/mm; ACROSS mean {mean_across:.4} \
             p90 {p90_across:.4} mm/mm; across/along mean ratio {:.2}x",
            along.len(),
            mean_across / mean_along.max(1e-9)
        );
    }
    ...
    const W_SWEEP: [f64; 6] = [0.0, 2.0, 8.0, 16.0, 32.0, 64.0];
    ...
    {
        let out = std::path::Path::new("target/graded_raster_e2");
        ...
        eprintln!("  SVGs written to target/graded_raster_e2/ (crop at [{crop:?}])");
    }
    ...
    for (label, runs) in &arm_list {
        let simplified: Vec<Vec<P3>> = runs
            .iter()
            .map(|r| simplify_run(r, OP_TOLERANCE_MM))
            .collect();
        let raw = toolpath_from_runs(&simplified, safe_z);
        let pre = pre_relink_fragments(&raw);
        let cost = relink_and_cost_under(raw, &mesh, &index, &r15, &boundary, &kinematics, &regime);
        let coverage = audit_arm_coverage(&tris, &cost.toolpath, r15.cusp_radius_mm(), bbox);
        eprintln!(
            "     {:>34}  {:>8}  {:>9}  {:>7}  {:>8}  {:>9.1}  {:>9.0}",
            label,
            pre,
