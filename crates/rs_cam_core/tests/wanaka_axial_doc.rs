...
fn wanaka_back_rough_axial_doc() {
    ...
    println!("loaded {} setups, {} toolpaths",
        session.list_setups().len(), session.list_toolpaths().len());
    ...
    for s in &cut_trace.samples {
        if s.toolpath_id != tp_id { continue; }
        match peak {
    ...
    println!("peak axial DOC: {:.3}mm at sample {} (move {})",
        peak.axial_doc_mm, peak.sample_index, peak.move_index);
    println!("  position: ({:.3}, {:.3}, {:.3})  arc {:.3}  chip_eff {:.4}",
    ...
    for j in lo..=hi {
        ...
        println!("    move {}: type={:?} target=({:.3},{:.3},{:.3})",
    ...
    println!("  total moves: {}, peak at move {} ({:.1}% through)",
    ...
    // in the toolpath.
    {
        ...
        let _ = probe.simulate_toolpath_with_metrics_with_cancel(
            &up_to, &probe_cutter, StockCutDirection::FromTop,
            tp_id, 18000, 2, 5000.0, 0.5, None, true, &probe_cancel,
        );
        ...
        let x = probe.z_grid.origin_u + col as f64 * cs;
        let y = probe.z_grid.origin_v + row as f64 * cs;
        println!("  PROBE before move {}: peak ray_top in footprint at row={} col={} (world {:.2},{:.2}): top={:.2}",
    ...
    for &(tx, ty, tz) in &perim_targets {
        ...
        for (j, m) in tp_result.toolpath.moves.iter().enumerate() {
            ...
            {
                if found_count == 0 { found_idx = j; }
                found_count += 1;
            }
        }
        println!("    target ({:.2}, {:.2}, {:.2}): {} match(es), first @ move {}", tx, ty, tz, found_count, found_idx);
    }
    ...
    for m in &tp_result.toolpath.moves {
        if matches!(m.move_type, rs_cam_core::toolpath::MoveType::Linear { .. } | rs_cam_core::toolpath::MoveType::ArcCW { .. } | rs_cam_core::toolpath::MoveType::ArcCCW { .. })
    ...
    println!("  feed/arc moves at z≈22 within 5mm of {:?}: {}", target, z22_near_count);
    ...
    for (j, m) in tp_result.toolpath.moves.iter().enumerate() {
        ...
        if Some(bucket) != prev_z_bucket {
            if let Some(b) = prev_z_bucket {
                println!("    moves {}..{}: ~z={:.1}", bucket_start, j - 1, b as f64 * 3.0);
            }
            bucket_start = j;
    ...
    if let Some(b) = prev_z_bucket {
        ...
        println!("    moves {}..{}: ~z={:.1}", bucket_start, last, b as f64 * 3.0);
    }
}
