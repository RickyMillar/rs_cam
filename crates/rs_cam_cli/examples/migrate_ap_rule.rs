...
const AP_RULE_MAPPING: &[(&str, Option<f64>, Option<f64>)] = &[
    ...
    ("cut depth per pass = cutting edge diameter (1xD); 2xD reduce chip load 25%; 3xD reduce chip load 50%", None, Some(1.0)),
    ("1xD use recommended feed rate; 2xD reduce 25%; 3xD reduce 50%", None, Some(1.0)),
    ("1xD use recommended chip load; 2xD reduce chip load 25%; 3xD reduce 50%", None, Some(1.0)),
    ("depth of cut equal to bit diameter; if 2xD reduce chipload by at least 25%; if 3xD reduce by at least 50%", None, Some(1.0)),
    ("1xD use recommended feed rate; 2xD reduce feed rate 25%; 3xD reduce 50%", None, Some(1.0)),
    ("1xD per chart; 3D-finish DOC unspecified by source", None, Some(1.0)),
    ("Profiling Axial = 1xD", None, Some(1.0)),
    ("Traditional ADOC = 0.500 in = 100% x D", None, Some(1.0)),

    // HEM-class rows (2×D).
    ("Profiling Axial = 2xD (HEM); side milling up to 2xD", None, Some(2.0)),
    ("HEM ADOC = 1.000 in = 200% x D", None, Some(2.0)),

    // Slotting-specific.
    ("Axial = .5xD (slotting); Pocket/Slot up to 1xD", None, Some(1.0)),
    ("Axial = .5xD (slotting)", None, Some(0.5)),

    // Plunge-only (20% D).
    ("20% of diameter for basic engagement parameters; drop feed ~50% when plunging into solid", None, Some(0.2)),
    ...
    ("side-entry or ramp entry required (no straight plunge); upcut O-flute for chip evacuation. Phase 5 promotion (2026-06-01) — diameter_mm omitted; article publishes one chipload window across the upcut O-flute line.", None, None),
];
...
fn migrate_file(
    ...
    for obs in obs_array.iter_mut() {
        ...
        let Some(ap_rule) = obj.get("ap_rule").and_then(Value::as_str).map(str::to_owned) else {
            continue;
        };
        stats.rows_with_rule += 1;

        let already_populated = obj.contains_key("ap_min_factor")
            || obj.contains_key("ap_max_factor");
