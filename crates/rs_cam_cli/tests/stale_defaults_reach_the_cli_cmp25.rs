//! Sentry: the stale-default validator reaches the batch CLI (CMP-25,
//! 2026-09-18).
//!
//! ## The gap this pins
//!
...
#[test]
fn the_project_command_runs_the_stale_default_validator_cmp25() {
    ...
    assert!(
        code.contains("stale_defaults: Vec<rs_cam_core::compute::validate::StaleDefault>"),
        "ProjectSummary no longer carries the validator's findings, so the \
         call above has nowhere to land in the artifact."
    );
    assert!(
        code.contains("stale_defaults,"),
        "ProjectSummary is built without its `stale_defaults` field, so the \
